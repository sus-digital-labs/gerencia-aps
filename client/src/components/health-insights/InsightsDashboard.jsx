...
import { Brain, AlertTriangle, CheckCircle2, Clock, Target, Lightbulb } from 'lucide-react';
import { toast } from 'sonner';
import { motion } from 'framer-motion';

const INSIGHT_TYPES = {
  risk_alert: { label: 'Alerta de Risco', icon: AlertTriangle, color: 'text-red-600' },
  preventive_action: { label: 'Ação Preventiva', icon: Target, color: 'text-blue-600' },
  follow_up: { label: 'Acompanhamento', icon: Clock, color: 'text-orange-600' },
  vaccination: { label: 'Vacinação', icon: CheckCircle2, color: 'text-green-600' },
...
export default function InsightsDashboard() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }) => trpc.HealthInsight.update(id, { 
      status,
  ...
  const stats = {
    ...
    urgente: insights.filter(i => i.priority === 'urgente' && i.status === 'pendente').length
  };
        ...
        <TabsContent value={activeTab} className="space-y-4 mt-6">
          {filteredInsights.length === 0 ? (
            ...
            filteredInsights.map((insight, idx) => {
              const typeConfig = INSIGHT_TYPES[insight.insight_type] || INSIGHT_TYPES.preventive_action;
                ...
                >
                  <Card className="border-0 shadow-lg hover:shadow-xl transition-all">
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        <p className="text-sm font-medium text-gray-700 mb-2">Cidadão</p>
                      ...
                      {insight.ai_confidence && (
                        <div>
                          <p className="text-xs text-gray-500">Confiança da IA: {insight.ai_confidence}%</p>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 pt-2">
                        {insight.status === 'pendente' && (
                          <Button 
                            size="sm" 
                            onClick={() => updateStatusMutation.mutate({ id: insight.id, status: 'em_andamento' })}
                          >
                            Iniciar
                          </Button>
                        )}
                        {insight.status === 'em_andamento' && (
                          <Button 
                            size="sm" 
                            onClick={() => updateStatusMutation.mutate({ id: insight.id, status: 'concluido' })}
                          >
                            Concluir
                          </Button>
                        )}
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => updateStatusMutation.mutate({ id: insight.id, status: 'ignorado' })}
                        >
                          Ignorar
                        </Button>
                      </div>
