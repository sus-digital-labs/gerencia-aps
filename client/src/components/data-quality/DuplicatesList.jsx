...
export default function DuplicatesList() {
    ...
    mutationFn: async (data) => {
      ...
      const edits = data.citizenIdsToDeactivate.map(citizenId => 
        trpc.PendingEdit.create({
          entity_type: 'citizen',
          entity_id: citizenId,
          citizen_name: citizens.find(c => c.id === citizenId)?.no_cidadao || '',
          field_name: 'st_ativo',
          old_value: '1',
          new_value: '0',
  ...
  const discardGroup = useMutation({
    mutationFn: (groupId) => trpc.DuplicateGroup.update(groupId, { 
      status: 'descartado',
      resolved_at: new Date().toISOString()
    }),
    onSuccess: () => {
  ...
  const getCitizensInGroup = (group) => {
    return group.citizen_ids?.map(id => citizens.find(c => c.id === id)).filter(Boolean) || [];
  };
  ...
  const handleUnify = () => {
    ...
    const citizenIdsToDeactivate = selectedGroup.citizen_ids.filter(id => id !== primaryCitizenId);
