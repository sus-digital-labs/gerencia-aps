...
export default function PeopleCalculator() {
  ...
  const stats = useMemo(() => {
    ...
    // By condition
    const withHypertension = active.filter(c => c.conditions?.includes('hipertensao')).length;
    const withDiabetes = active.filter(c => c.conditions?.includes('diabetes')).length;
    const pregnant = active.filter(c => c.conditions?.includes('gestante')).length;
