...
export default function CardiovascularRiskPage() {
  ...
  const [carePlanDialog, setCarePlanDialog] = useState({ open: false, patient: null });
  ...
  const calculateRisk = (citizen) => {
    let score = 0;
    const age = citizen.age || (citizen.dt_nascimento ? Math.floor((new Date() - new Date(citizen.dt_nascimento)) / 31536000000) : 50);
  ...
  const runCalculation = async () => {
    ...
    try {
      ...
      const eligibleCitizens = citizens.filter(c => {
        const age = c.dt_nascimento ? Math.floor((new Date() - new Date(c.dt_nascimento)) / 31536000000) : 0;
  ...
  const filteredData = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(r => r.citizen_name?.toLowerCase().includes(term) || r.citizen_cns?.includes(term));
    }
    return data;
  ...
  const stats = useMemo(() => {
    ...
    const intermediario = riskData.filter(r => r.risk_level === 'intermediario').length;
    const alto = riskData.filter(r => r.risk_level === 'alto').length;
    const emAcompanhamento = riskData.filter(r => r.care_plan_status === 'em_acompanhamento').length;
  ...
  const ageGroupData = useMemo(() => {
    ...
    riskData.filter(r => r.risk_level === 'alto').forEach(r => {
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      onClick={(data) => setSelectedRisk(data.name === 'Baixo Risco' ? 'baixo' : data.name === 'Risco Intermediário' ? 'intermediario' : 'alto')}
