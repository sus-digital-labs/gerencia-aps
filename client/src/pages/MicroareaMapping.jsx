...
const statusCor = (s) => s === 'excesso' ? '#EF4444' : s === 'baixa_cobertura' ? '#F59E0B' : '#10B981';
const statusLabel = (s) => s === 'excesso' ? 'Excesso' : s === 'baixa_cobertura' ? 'Baixa Cobertura' : s === 'inativa' ? 'Inativa' : 'Normal';
...
export default function MicroareaMapping() {
  ...
  async function importarKML(event) {
    ...
    try {
      ...
      msg(`${areas.length} área(s) importada(s) do ${isKmz ? 'KMZ' : 'KML'}. Clique em "Aplicar" para salvar.`);
    } catch (err) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 3.5rem)', background: '#f8fafc', fontFamily: 'Inter, sans-serif', overflow: 'hidden' }}>
      ...
      {kmlImportado && (
        <div style={{ background: '#EDE9FE', borderBottom: '1px solid #C4B5FD', padding: '10px 24px', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          <span style={{ fontSize: '14px', color: '#5B21B6' }}>📂 <strong>{kmlImportado.length} área(s)</strong> carregada(s) do arquivo KMZ/KML.</span>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
            <div style={{ flex: 1, position: 'relative', minHeight: 0 }}>
              <MapContainer center={mapCenter} zoom={13} style={{ width: '100%', height: '100%' }}>
                ...
                {kmlImportado && kmlImportado.map((area, i) => {
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#f8fafc' }}>
                        {['', 'Nome', 'ACS', 'Famílias', 'Cidadãos', '% Limite', 'Status', ''].map((h, i) => (
