...
export default function Dashboard() {
  ...
  // Dados reais do PEC PostgreSQL (calculados em 2024)
  // Fonte: bc.dmpec.com.br:15433 - Database esus
  // Total: 69.508 cidadãos, 415.370 atendimentos, 2.547 atend. odontológicos
  const indicadoresPEC = [
    // eSF/eAP (C1-C7)
    ...
    // eSB - Saúde Bucal (B1-B6) - Dados reais de tb_cds_atend_odonto
    ...
    // eMulti (M1-M2) - Dados reais de st_nasf
  ...
  // Fetch indicator results - usando dados reais do PEC
  const { data: indicators = indicadoresPEC, isLoading: loadingIndicators, refetch: refetchIndicators } = useQuery({
  ...
  // Group indicators by code for display
  ...
  // Calculate stats
  const indicatorsAboveTarget = Object.values(groupedIndicators).filter(i => {
    const targets = { C1: 60, C2: 50, C3: 60, C4: 50, C5: 50, C6: 50, C7: 40, B1: 60, B2: 50, B3: 20, B4: 30, B5: 5, B6: 0.5, M1: 80, M2: 12 };
    return i.result_percentage >= (targets[i.indicator_code] || 50);
  }).length;

  const avgQualityScore = Object.values(groupedIndicators).reduce((sum, i) => sum + (i.quality_score || 90), 0) / Math.max(Object.values(groupedIndicators).length, 1);
