...
export default function IndicatorDetail() {
  ...
  const initialMonth = parseInt(urlParams.get('month')) || new Date().getMonth() + 1;
  ...
  // Fetch main indicator result
  const { data: indicatorResults = [], isLoading, refetch } = useQuery({
  ...
  const { data: historicalData = [] } = useQuery({
    ...
    queryFn: async () => {
      const query = { indicator_code: indicatorCode };
      if (filters.teamId) query.team_id = filters.teamId;
  ...
  // Fetch comparison data (by unit or team)
  ...
  // Fetch citizen status (nominal list) - Dados reais do PEC via LEDI
  const { data: citizenStatus = [] } = useQuery({
    queryKey: ['citizenStatus', indicatorCode, filters],
    queryFn: async () => {
      // Tentar buscar dados reais do PEC primeiro
      try {
        ...
        if (data.result?.data?.cidadaos) {
          return data.result.data.cidadaos.map(c => ({
            citizen_cns: c.cns || '',
            citizen_name: c.nome,
            citizen_cpf: c.cpf,
  ...
  // Fetch units and teams for filters
  ...
  const handleOpenProntuario = (citizen) => {
    // In a real implementation, this would open the e-SUS PEC prontuário
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        {aggregatedResult && (
          <IndicatorDetailHeader
            ...
            teamName={filters.teamId ? teams.find(t => t.id === filters.teamId)?.name : null}
            unitName={filters.unitId ? units.find(u => u.id === filters.unitId)?.name : null}
