...
export default function SavedSearches({ onApplySearch }: SavedSearchesProps) {
  ...
  const { data: savedSearches = [] } = useQuery<SavedSearch[]>({
    ...
    queryFn: () => trpc.SavedSearch.filter({}, '-created_date') as any
  });

  const createMutation = useMutation({
    mutationFn: (data: Partial<SavedSearch>) => trpc.SavedSearch.create(data as any),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string | number) => trpc.SavedSearch.delete(id as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['savedSearches'] });
  ...
  const toggleFavoriteMutation = useMutation({
    mutationFn: ({ id, is_favorite }: { id: string | number; is_favorite: boolean }) => 
      trpc.SavedSearch.update(id as any, { is_favorite } as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['savedSearches'] });
    }
  });

  const handleSaveSearch = (filters: any, searchTerm: string) => {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {savedSearches.length === 0 ? (
          <p className="text-sm text-gray-500 text-center py-4">Nenhuma busca salva</p>
        ) : (
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                >
                  ...
                  <Badge variant="outline" className="text-xs mt-1">
                    {search.search_type === 'global' ? 'Busca Global' : 'Busca Ativa'}
                  </Badge>
                </button>
              </div>
              <Button
