...
export interface VaccinationCoverageProps {
  data?: any[];
}

export default function VaccinationCoverage({ data = [] }: VaccinationCoverageProps) {
  const calculateCoverage = (vaccinated: number, total: number) => {
    return total > 0 ? ((vaccinated / total) * 100).toFixed(1) : "0.0";
  };
    ...
    <Card className="border-0 shadow-lg">
      ...
      <CardContent className="p-6 space-y-4">
        {vaccines.map((vaccine, idx) => {
          ...
          const isGood = Number(coverage) >= 80;
