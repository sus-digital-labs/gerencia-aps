...
export interface HomeVisitsByConditionProps {
  visits?: any[];
}

export default function HomeVisitsByCondition({ visits = [] }: HomeVisitsByConditionProps) {
  const data = [
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-500">Meta: {payload[0].payload.target}</p>
          <p className="text-xs text-gray-400">
            {((payload[0].value / payload[0].payload.target) * 100).toFixed(0)}% da meta
          </p>
        </div>
      );
    }
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {data.map((item, idx) => {
            ...
            const isGood = Number(percentage) >= 80;
            ...
            return (
              <div key={idx} className="p-3 bg-gray-50 rounded-lg">
                ...
                <Badge className={`text-xs mt-1 ${isGood ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                  {percentage}% da meta
                </Badge>
              </div>
            );
