...
export default function TaskCompletedHistory({ tasks = [], acsList = [] }) {
  ...
  const handleExport = () => {
    ...
    const rows = completedTasks.map(t => [
      ...
      t.related_address || "-",
      t.completed_at
        ? format(new Date(t.completed_at), "dd/MM/yyyy HH:mm")
        : "-",
      t.completion_notes || "-",
    ]);
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {stats.byType["visita_risco_cv"] || 0}
                </p>
              </div>
            </div>
          </CardContent>
                  ...
                  return (
                    <TableRow key={task.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <Badge
                          className={
                            TASK_TYPES[task.task_type]?.color || "bg-gray-100"
                          }
                        >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2">
              <p className="text-sm text-gray-500">
                ...
                {Math.min(page * pageSize, completedTasks.length)} de{" "}
          ...
          {selectedTask && (
            <div className="space-y-4">
              <div>
                ...
                <Badge
                  className={
                    TASK_TYPES[selectedTask.task_type]?.color || "bg-gray-100"
                  }
                >
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {selectedTask.related_citizen_name || "-"}
                  </p>
                </div>
                <div>
                  <p className="text-gray-500">Endereço</p>
                  <p className="font-medium">
                    {selectedTask.related_address || "-"}
                  </p>
                </div>
              </div>
                ...
                {selectedTask.latitude && selectedTask.longitude ? (
                  <div className="text-sm space-y-1">
                    <p>
                      ...
                      {selectedTask.latitude.toFixed(6)},{" "}
                      {selectedTask.longitude.toFixed(6)}
                    </p>
                    {selectedTask.expected_latitude &&
                      selectedTask.expected_longitude && (
                        <>
                          <p>
                            <span className="text-gray-500">
                              Localização esperada:
                            </span>{" "}
                            {selectedTask.expected_latitude.toFixed(6)},{" "}
                            {selectedTask.expected_longitude.toFixed(6)}
                          </p>
                          {(() => {
