...
import { 
  Search, AlertTriangle, Eye, ChevronLeft, ChevronRight, 
  FileText, Calendar, MapPin, User
} from 'lucide-react';
...
const desfechoColors: Record<string, string> = {
...
export interface VisitsTableProps {
  visits?: TableVisit[];
  onViewDetails?: (visit: TableVisit) => void;
  pageSize?: number;
}
...
export default function VisitsTable({ 
  ...
  const hasInconsistency = (visit: TableVisit) => {
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50">
                ...
                <TableHead className="font-semibold">Responsável Familiar</TableHead>
                ...
                <TableRow key={visit.id || idx} className="hover:bg-gray-50">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium">
                          {visit.visit_date ? format(new Date(visit.visit_date), 'dd/MM/yyyy', { locale: ptBR }) : '-'}
                        </p>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium truncate max-w-[150px]" title={visit.citizen_name}>
                        {visit.citizen_name || '-'}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate max-w-[180px]" title={visit.address}>
                        {visit.address || '-'}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
                    ...
                    <p className="text-xs text-gray-500">MA: {visit.microarea || '-'}</p>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge className={`text-xs ${desfechoColors[visit.desfecho] || 'bg-gray-100 text-gray-700'}`}>
                      {desfechoLabels[visit.desfecho] || visit.desfecho}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-center">
                    {hasInconsistency(visit) && (
                      <AlertTriangle className="w-4 h-4 text-amber-500 mx-auto" title="Pendências cadastrais" />
                    )}
                  </TableCell>
                  <TableCell className="text-right">
                    <Link to={createPageUrl(`ACSTimeline?visit_id=${visit.id}&acs_id=${visit.acs_id}&date=${visit.visit_date}`)}>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4">
            <p className="text-sm text-gray-500">
              Mostrando {(currentPage - 1) * pageSize + 1} a {Math.min(currentPage * pageSize, filteredVisits.length)} de {filteredVisits.length}
            </p>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
