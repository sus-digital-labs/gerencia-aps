...
export default function TaskGeolocationComplete({
  task,
  onClose,
  onComplete,
  mode = "complete",
  ...
  const updateTaskMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        isStarting ? "Tarefa iniciada!" : "Tarefa concluída com sucesso!"
      );
  ...
  return (
    <Card className="shadow-xl border-0 max-w-lg mx-auto">
      <CardHeader
        className={`bg-gradient-to-r ${isStarting ? "from-blue-600 to-cyan-600" : "from-emerald-600 to-teal-600"} text-white rounded-t-lg`}
      >
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          {task.related_citizen_name && (
            <p className="text-sm text-gray-600">
              Cidadão: {task.related_citizen_name}
            </p>
          )}
        ...
        {!isStarting && task.start_time && (
          <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <div className="text-sm text-blue-700 space-y-1">
              <p>
                Início: {format(new Date(task.start_time), "dd/MM/yyyy HH:mm")}
              </p>
              {task.start_latitude && <p>Local de início registrado ✓</p>}
            </div>
          </div>
        )}
                  ...
                  >
                    ...
                    {!validationResult.valid && (
                      <span className="block text-xs mt-1">
                        A tarefa ainda pode ser concluída, mas será sinalizada
                        para revisão.
                      </span>
                    )}
                  </AlertDescription>
        ...
        <div className="space-y-2">
          <Label>
            {isStarting ? "Observações" : "Observações da Conclusão"}
          </Label>
          <Textarea
            value={notes}
            ...
            placeholder={
              isStarting
                ? "Observações ao iniciar a tarefa..."
        ...
        {!location && !isGettingLocation && (
          <p className="text-xs text-amber-600 text-center">
            ⚠️ Sem localização, a tarefa será{" "}
            {isStarting ? "iniciada" : "concluída"} sem validação geográfica
          </p>
        )}
      </CardContent>
    </Card>
