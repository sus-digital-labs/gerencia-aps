...
import { 
  Navigation, MapPin, Clock, Route, Smartphone, 
  QrCode, Copy, Check, X
} from 'lucide-react';
...
interface RouteOptimizerProps {
  visits?: Visit[];
  onRouteOptimized?: (route: Point[]) => void;
  onClose?: () => void;
}
...
function optimizeRoute(points: Point[]): Point[] {
  ...
  let current = remaining.shift() as Point;
  result.push(current);
...
function calculateEstimatedTime(points: Point[]): number {
...
function generateGoogleMapsUrl(points: Point[]): string {
...
export default function RouteOptimizer({ 
  ...
  const handleOptimize = () => {
    const points: Point[] = selectedVisits.map(v => ({
      id: v.id,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-medium text-gray-700">Selecione as visitas</h4>
                  ...
                  {visitsWithCoords.map(visit => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm truncate">{visit.citizen_name}</p>
                ...
                <ScrollArea className="h-48 border rounded-lg p-2">
                  <div className="space-y-2">
                    {optimizedRoute.map((point, idx) => (
                      <div key={point.id} className="flex items-center gap-3 p-2">
                        <Badge className={`w-6 h-6 flex items-center justify-center ${
                          idx === 0 ? 'bg-green-500' : 
                          idx === optimizedRoute.length - 1 ? 'bg-red-500' : 
                          'bg-blue-500'
                        }`}>
                          {idx + 1}
                        </Badge>
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="p-4 bg-white border rounded-lg">
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(mapsUrl)}`}
                  alt="QR Code da Rota"
                  className="w-48 h-48"
                />
              </div>
