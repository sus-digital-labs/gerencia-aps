...
export interface TaskTrajectoryReportProps {
  task: any;
}

export default function TaskTrajectoryReport({ task }: TaskTrajectoryReportProps) {
  ...
  const allPoints = [
    { lat: task.start_latitude, lng: task.start_longitude, type: 'start' as const },
    ...locations.map((l: any) => ({ lat: l.latitude, lng: l.longitude, type: l.location_type, timestamp: l.timestamp })),
    { lat: task.latitude, lng: task.longitude, type: 'end' as const }
  ];
  ...
  const pathCoordinates: [number, number][] = allPoints.map(p => [p.lat, p.lng]);
  ...
  const duration = task.completed_at && task.start_time
    ? new Date(task.completed_at).getTime() - new Date(task.start_time).getTime()
    : 0;
      ...
      <CardContent className="p-0">
        ...
        <div className="h-[500px]">
          <MapContainer center={center} zoom={15} style={{ height: '100%', width: '100%' }}>
            ...
            {/* Checkpoint Markers */}
            {locations.filter((l: any) => l.location_type === 'checkpoint').map((loc: any, idx: number) => (
