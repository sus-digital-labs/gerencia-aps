...
export interface TaskGeolocationCompleteProps {
  task: Task;
  onClose: () => void;
  onComplete?: () => void;
  mode?: 'start' | 'complete';
...
export default function TaskGeolocationComplete({ 
  ...
  onComplete, 
  mode = 'complete' 
}: TaskGeolocationCompleteProps) {
  const [location, setLocation] = useState<LocationData | null>(null);
  const [locationError, setLocationError] = useState<string | null>(null);
  const [isGettingLocation, setIsGettingLocation] = useState(false);
  const [notes, setNotes] = useState('');
  const [validationResult, setValidationResult] = useState<{ distance: number; valid: boolean; message: string } | null>(null);
  ...
  const updateTaskMutation = useMutation({
    mutationFn: (data: any) => trpc.ACSTask.update(task.id, data),
    onSuccess: () => {
