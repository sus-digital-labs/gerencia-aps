...
export interface VisitsFiltersProps {
  filters: any;
...
export default function VisitsFilters({ 
  ...
  const filteredTeams = filters.unit_id && filters.unit_id !== 'all'
    ? teams.filter(t => t.unit_id === filters.unit_id)
    : teams;

  // Filter ACS by selected team
  const filteredACS = filters.team_id && filters.team_id !== 'all'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(v) => onFilterChange({ ...filters, acs_id: v === 'all' ? '' : v })}
            >
              <SelectTrigger className="h-9">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={onReset} className="h-9 gap-2">
              <X className="w-4 h-4" />
              Limpar
            </Button>
          </div>
