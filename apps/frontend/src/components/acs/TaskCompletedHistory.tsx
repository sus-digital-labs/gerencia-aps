...
import { 
  CheckCircle2, User, MapPin, Clock, Search, 
  Eye, Download, Navigation, AlertTriangle
} from 'lucide-react';
...
export interface TaskCompletedHistoryProps {
  tasks?: Task[];
  acsList?: ACS[];
}

export default function TaskCompletedHistory({ tasks = [], acsList = [] }: TaskCompletedHistoryProps) {
  ...
  const completedTasks = useMemo(() => {
    return tasks
      ...
      .sort((a, b) => {
        const dateA = a.completed_at || a.updated_date || '';
        const dateB = b.completed_at || b.updated_date || '';
  ...
  const stats = useMemo(() => {
    ...
    const byACS: Record<string, number> = {};
  ...
  const validateLocation = (task: Task) => {
