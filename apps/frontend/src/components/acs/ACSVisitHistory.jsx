...
export default function ACSVisitHistory({ acs, visits = [] }) {
  ...
  const filteredVisits = useMemo(() => {
    ...
    return data.sort((a, b) => new Date(b.visit_date) - new Date(a.visit_date));
  }, [acs, visits, filters]);
  ...
  const stats = useMemo(() => {
    ...
    const realizadas = filteredVisits.filter(
      v => v.desfecho === "visita_realizada"
    ).length;
    const ausentes = filteredVisits.filter(
      v => v.desfecho === "ausente"
    ).length;
    return {
      total,
      realizadas,
      ausentes,
      taxaSucesso: total > 0 ? Math.round((realizadas / total) * 100) : 0,
    };
  }, [filteredVisits]);
        ...
        <CardContent className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <Select
              ...
              onValueChange={v => setFilters({ ...filters, visitType: v })}
            >
              <SelectTrigger>
              ...
              {paginatedVisits.map((visit, idx) => (
                <TableRow key={visit.id || idx} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    ...
                    >
                      {DESFECHO_LABELS[visit.desfecho] ||
                        visit.desfecho ||
                        "Realizada"}
                    </Badge>
                  </TableCell>
