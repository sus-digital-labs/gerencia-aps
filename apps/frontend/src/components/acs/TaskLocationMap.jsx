...
export default function TaskLocationMap({ task }) {
    ...
    return (
      <Card className="border-0 shadow-lg">
        <CardContent className="p-12 text-center text-gray-500">
          ...
          <p className="text-sm">
            Esta tarefa não possui coordenadas de geolocalização
          </p>
        </CardContent>
      </Card>
    );
  ...
  const center = hasExpectedLocation
    ? [
        (task.latitude + task.expected_latitude) / 2,
        (task.longitude + task.expected_longitude) / 2,
      ]
        ...
        <div className="h-[400px]">
          ...
          >
            ...
            {hasExpectedLocation && (
              <>
                <Marker
                  position={[task.expected_latitude, task.expected_longitude]}
                  icon={expectedIcon}
                >
                  <Popup>
                    <div className="min-w-[200px]">
                      ...
                      <p className="text-sm text-gray-600">
                        {task.related_address || "Endereço não informado"}
                      </p>
            ...
            >
              <Popup>
                <div className="min-w-[200px]">
                  <h4
                    className={`font-bold mb-2 ${isValid ? "text-green-700" : "text-red-700"}`}
                  >
                    {isValid
                      ? "✅ Local Registrado"
                      : "⚠️ Local Registrado (Fora do Esperado)"}
                  </h4>
