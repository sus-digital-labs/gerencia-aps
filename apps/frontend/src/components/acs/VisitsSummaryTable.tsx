...
export interface VisitsSummaryTableProps {
  visits?: VisitSummary[];
  acsName?: string;
}

export default function VisitsSummaryTable({ visits = [], acsName }: VisitsSummaryTableProps) {
  ...
  const countVisits = (startDate: Date, endDate = new Date()) => {
  ...
  const visitsByType = visits.filter(v => new Date(v.visit_date) >= startOfMonth).reduce((acc: Record<string, number>, v) => {
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-emerald-50 rounded-xl p-4 text-center">
              ...
              <p className="text-3xl font-black text-emerald-700">{monthCount}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="flex items-center gap-4">
                <div className="text-4xl font-black text-gray-800">{avgPerDay}</div>
            ...
            {/* Por tipo */}
            <div className="bg-gray-50 rounded-xl p-4">
              <h4 className="font-semibold text-gray-700 mb-3">Por Tipo (Mês)</h4>
              <div className="flex flex-wrap gap-2">
                ...
                {Object.keys(visitsByType).length === 0 && (
                  <span className="text-sm text-gray-400">Nenhuma visita no período</span>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Tabela detalhada */}
          <div className="mt-6">
            <h4 className="font-semibold text-gray-700 mb-3">Últimas 10 Visitas</h4>
                ...
                <TableBody>
                  {visits.slice(0, 10).map((visit, idx) => (
                    <TableRow key={visit.id || idx}>
                      ...
                      <TableCell>
                        <Badge className={
                          visit.desfecho === 'visita_realizada' ? 'bg-green-100 text-green-700' :
                          visit.desfecho === 'ausente' ? 'bg-amber-100 text-amber-700' :
