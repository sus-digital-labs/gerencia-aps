...
import { 
  Users, Filter, ClipboardList, Heart, Bug, UserPlus,
...
const TASK_TYPES = [
  { value: 'visita_risco_cv', label: 'Visitar Paciente Alto Risco CV', icon: Heart },
...
const CONDITION_FILTERS = [
  { value: 'alto_risco_cv', label: 'Alto Risco Cardiovascular', color: 'bg-rose-100 text-rose-700' },
  { value: 'hipertenso', label: 'Hipertenso', color: 'bg-red-100 text-red-700' },
  { value: 'diabetico', label: 'Diabético', color: 'bg-amber-100 text-amber-700' },
  { value: 'gestante', label: 'Gestante', color: 'bg-pink-100 text-pink-700' },
  { value: 'idoso', label: 'Idoso (60+)', color: 'bg-purple-100 text-purple-700' },
  { value: 'crianca', label: 'Criança (0-5)', color: 'bg-green-100 text-green-700' },
];
...
export interface CVRiskPatient {
  id: string;
  citizen_name: string;
  citizen_cns?: string;
  risk_level?: string;
...
export interface TaskBulkAssignmentProps {
  acsList?: ACS[];
  citizens?: Citizen[];
  cvRiskPatients?: CVRiskPatient[];
  areas?: any[];
}
...
export default function TaskBulkAssignment({ 
  ...
  const filteredCitizens = useMemo(() => {
    ...
    if (filters.microarea && filters.microarea !== 'null') {
      result = result.filter(c => c.microarea === filters.microarea);
    }
    if (filters.neighborhood) {
      result = result.filter(c => c.neighborhood?.toLowerCase().includes(filters.neighborhood.toLowerCase()));
    }
    if (filters.care_plan_status && filters.care_plan_status !== 'null') {
      result = result.filter(c => c.care_plan_status === filters.care_plan_status);
    }
    if (filters.without_visit_days) {
      const cutoff = new Date();
      cutoff.setDate(cutoff.getDate() - filters.without_visit_days);
      result = result.filter(c => !c.last_visit_date || new Date(c.last_visit_date) < cutoff);
    }
  ...
  const handleCreateTasks = async () => {
    ...
    const citizensToProcess = filteredCitizens.filter(c => selectedCitizens.includes(c.id));
    const tasksToCreate = [];
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          {[1, 2, 3].map(s => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={`text-sm ${step >= s ? 'text-indigo-600 font-medium' : 'text-gray-400'}`}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Select value={filters.condition} onValueChange={(v) => setFilters({ ...filters, condition: v === 'null' ? '' : v })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select value={filters.microarea} onValueChange={(v) => setFilters({ ...filters, microarea: v === 'null' ? '' : v })}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="null">Todas</SelectItem>
                    {microareas.map(m => (
                      <SelectItem key={m} value={m}>Microárea {m}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFilters({ ...filters, without_visit_days: parseInt(e.target.value) || 0 })}
                  placeholder="30"
                />
              </div>
              ...
              {filters.condition === 'alto_risco_cv' && (
                <div className="space-y-2">
                  ...
                  <Select value={filters.care_plan_status} onValueChange={(v) => setFilters({ ...filters, care_plan_status: v === 'null' ? '' : v })}>
                    <SelectTrigger>
                ...
                {filteredCitizens.map(citizen => (
                  ...
                  >
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {citizen.conditions?.slice(0, 2).map((cond: string, i: number) => (
                        <Badge key={i} className="text-xs bg-gray-100 text-gray-600">
                          {cond}
                        </Badge>
                      ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(v) => setTaskConfig({ ...taskConfig, task_type: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(v) => setTaskConfig({ ...taskConfig, priority: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTaskConfig({ ...taskConfig, due_date: e.target.value })}
                />
              </div>
              ...
              <div className="space-y-2 flex items-end">
                <div className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(c) => setTaskConfig({ ...taskConfig, assign_to_acs: !!c })}
                  />
                  <Label className="cursor-pointer">Atribuir automaticamente ao ACS da microárea</Label>
                </div>
              </div>
            </div>
