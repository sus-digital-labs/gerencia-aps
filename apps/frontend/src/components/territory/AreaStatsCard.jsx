...
export default function AreaStatsCard({ area, citizens = [] }) {
  ...
  const areaCitizens = citizens.filter(
    c => c.microarea === area?.microarea_code
  );

  const stats = {
    total: areaCitizens.length,
    gestantes: areaCitizens.filter(c => c.conditions?.includes("gestante"))
      .length,
    hipertensos: areaCitizens.filter(c => c.conditions?.includes("hipertenso"))
      .length,
    diabeticos: areaCitizens.filter(c => c.conditions?.includes("diabetico"))
      .length,
    idosos: areaCitizens.filter(c => c.conditions?.includes("idoso")).length,
    criancas: areaCitizens.filter(c => c.conditions?.includes("crianca"))
      .length,
  };
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50 rounded-xl p-3 text-center">
              ...
              <p className="text-2xl font-black text-blue-700">
                {area.total_citizens || stats.total}
              </p>
