...
export default function NominalList({
  ...
  const getStatusBadge = status => {
    switch (status) {
      ...
      default:
        return (
          <Badge className="bg-gray-100 text-gray-700">Não Elegível</Badge>
        );
    }
  };
    ...
    <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
      <CardHeader className="border-b bg-gray-50">
        ...
        <div className="flex gap-4 mt-4 text-sm text-gray-600">
          <span>
            Total: <strong>{citizens.length}</strong>
          </span>
          {type === "pending" && (
            <span className="text-amber-600">
              Com Pendências:{" "}
              <strong>
                {citizens.filter(c => c.status === "pendente").length}
              </strong>
            </span>
          )}
        </div>
              ...
              <AnimatePresence mode="popLayout">
                {paginatedCitizens.map((citizen, index) => (
                  ...
                  >
                    ...
                    {type === "pending" && (
                      <TableCell>
                        {citizen.pending_criteria?.length > 0 ? (
                          <ul className="text-sm space-y-1">
                            {citizen.pending_criteria.map((criteria, idx) => (
                              <li
                                key={idx}
                                className="flex items-start gap-1 text-red-600"
                              >
                    ...
                    <TableCell className="text-center text-sm text-slate-500">
                      {onOpenProntuario
                        ? "Ação autorizada pelo integrador"
                        : "Prontuário indisponível neste checkout"}
                    </TableCell>
                  </motion.tr>
                ))}
              </AnimatePresence>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t">
            <span className="text-sm text-gray-600">
              ...
              {Math.min(startIndex + itemsPerPage, filteredCitizens.length)} de{" "}
