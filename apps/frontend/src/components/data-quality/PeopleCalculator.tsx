...
import { 
  Calculator, Users, UserCheck, Heart, Activity, Baby, 
  User, PersonStanding, Loader2, Eye, Download
} from 'lucide-react';
...
export default function PeopleCalculator() {
  ...
  const [listCitizens, setListCitizens] = useState<any[]>([]);
  ...
  // Calculate stats
  const stats = useMemo(() => {
    const active = citizens.filter((c: any) => c.st_ativo === 1);
    
    // By sex
    const bySex = active.reduce((acc: any, c: any) => {
      const sex = c.co_dim_sexo === 1 ? 'Masculino' : c.co_dim_sexo === 2 ? 'Feminino' : 'Não informado';
      acc[sex] = (acc[sex] || 0) + 1;
      return acc;
    }, {});
    ...
    const byAge = active.reduce((acc: any, c: any) => {
      if (!c.dt_nascimento) {
    ...
    // By condition
    const withHypertension = active.filter((c: any) => c.conditions?.includes('hipertensao')).length;
    const withDiabetes = active.filter((c: any) => c.conditions?.includes('diabetes')).length;
    const pregnant = active.filter((c: any) => c.conditions?.includes('gestante')).length;
  ...
  const sexData = Object.entries(stats.bySex).map(([name, value]) => ({ name, value }));
  const ageData = Object.entries(stats.byAge)
    .sort((a, b) => {
      const order = ['0-4 anos', '5-9 anos', '10-19 anos', '20-59 anos', '60+ anos', 'Não informado'];
      return order.indexOf(a[0]) - order.indexOf(b[0]);
    })
    .map(([name, value]) => ({ name, value }));

  const handleShowList = (filter: string, title: string) => {
    const active = citizens.filter((c: any) => c.st_ativo === 1);
    let filtered: any[] = [];
    ...
    switch (filter) {
      case 'all':
        ...
        break;
      case 'male':
        filtered = active.filter((c: any) => c.co_dim_sexo === 1);
        break;
      case 'female':
        filtered = active.filter((c: any) => c.co_dim_sexo === 2);
        break;
      case 'hypertension':
        filtered = active.filter((c: any) => c.conditions?.includes('hipertensao'));
        break;
      case 'diabetes':
        filtered = active.filter((c: any) => c.conditions?.includes('diabetes'));
        break;
      case 'pregnant':
        filtered = active.filter((c: any) => c.conditions?.includes('gestante'));
        break;
      case 'elderly':
        const today = new Date();
        filtered = active.filter((c: any) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card 
            className="shadow-lg border-0 bg-white/90 hover:shadow-xl transition-shadow cursor-pointer"
            onClick={() => handleShowList('hypertension', 'Cidadãos com Hipertensão')}
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-red-600">{stats.withHypertension.toLocaleString()}</p>
          ...
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-amber-600">{stats.withDiabetes.toLocaleString()}</p>
          ...
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-pink-600">{stats.pregnant.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold">{(stats.bySex['Masculino'] || 0).toLocaleString()}</p>
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold">{(stats.bySex['Feminino'] || 0).toLocaleString()}</p>
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold">{(stats.byAge['60+ anos'] || 0).toLocaleString()}</p>
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold">{((stats.byAge['0-4 anos'] || 0) + (stats.byAge['5-9 anos'] || 0)).toLocaleString()}</p>
          ...
          <div className="overflow-auto flex-1">
              ...
              <TableBody>
                {listCitizens.map((c, idx) => (
                  <TableRow key={c.id || idx}>
                    <TableCell className="font-medium">{c.no_cidadao}</TableCell>
                    <TableCell>
                      {c.dt_nascimento ? new Date(c.dt_nascimento).toLocaleDateString('pt-BR') : '-'}
                    </TableCell>
                    <TableCell className="font-mono text-sm">{c.nu_cpf || '-'}</TableCell>
                    <TableCell className="font-mono text-sm">{c.nu_cns || '-'}</TableCell>
                    <TableCell>
                      {c.co_dim_sexo === 1 ? 'M' : c.co_dim_sexo === 2 ? 'F' : '-'}
                    </TableCell>
                  </TableRow>
            ...
            {listCitizens.length === 100 && (
              <p className="text-center text-sm text-gray-500 py-4">
                Mostrando os primeiros 100 registros. Exporte o CSV para ver todos.
              </p>
            )}
          </div>
        </DialogContent>
