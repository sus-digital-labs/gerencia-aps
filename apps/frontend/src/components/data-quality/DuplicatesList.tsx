...
import { Copy, Users, AlertTriangle, CheckCircle2, XCircle, Merge, Loader2 } from 'lucide-react';
...
export default function DuplicatesList() {
  ...
  const { data: duplicateGroups = [], isLoading } = useQuery({
    ...
    queryFn: () => trpc.DuplicateGroup.filter({ status: 'pendente' }, '-created_date')
  });
  ...
  const createPendingEdit = useMutation({
    mutationFn: async (data: any) => {
      ...
      const edits = data.citizenIdsToDeactivate.map((citizenId: string) => 
        trpc.PendingEdit.create({
          entity_type: 'citizen',
          entity_id: citizenId,
          citizen_name: citizens.find((c: any) => c.id === citizenId)?.no_cidadao || '',
          field_name: 'st_ativo',
          old_value: '1',
          new_value: '0',
  ...
  const discardGroup = useMutation({
    mutationFn: (groupId: string) => trpc.DuplicateGroup.update(groupId, { 
      status: 'descartado',
      resolved_at: new Date().toISOString()
    }),
    onSuccess: () => {
  ...
  const getCitizensInGroup = (group: any) => {
    return group.citizen_ids?.map((id: string) => citizens.find((c: any) => c.id === id)).filter(Boolean) || [];
  };
  ...
  const handleUnify = () => {
    ...
    const citizenIdsToDeactivate = selectedGroup.citizen_ids.filter((id: string) => id !== primaryCitizenId);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/80">
                {duplicateGroups.length} grupos de possíveis duplicados identificados
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {isLoading ? (
        ...
        <Card className="shadow-lg border-0 bg-white/90">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-lg font-medium text-gray-700">Nenhum cadastro duplicado pendente</p>
            <p className="text-sm text-gray-500">Todos os grupos foram resolvidos!</p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <Card className="shadow-lg border-0 bg-white/90 overflow-hidden">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge className="bg-amber-100 text-amber-700">
                            {matchTypeLabels[group.match_type] || group.match_type}
                          </Badge>
                        </div>
                      </div>
                      {group.mother_name && (
                        <p className="text-sm text-gray-600 mt-1">
                          ...
                          <strong> Nascimento:</strong> {group.birth_date ? new Date(group.birth_date).toLocaleDateString('pt-BR') : '-'}
                        </p>
                      )}
                    </CardHeader>
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {groupCitizens.map((citizen: any, cidx: number) => (
                          <div 
                            key={citizen.id}
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <p className="font-medium text-gray-800">{citizen.no_cidadao}</p>
                            ...
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">
                                  {citizen.dt_nascimento ? new Date(citizen.dt_nascimento).toLocaleDateString('pt-BR') : '-'}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">CPF:</span>
                                <span className="font-mono">{citizen.nu_cpf || '-'}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">CNS:</span>
                                <span className="font-mono">{citizen.nu_cns || '-'}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">Mãe:</span>
                                <span className="truncate max-w-32">{citizen.no_mae || '-'}</span>
                              </div>
                            </div>
                          </div>
                        ))}
      ...
      <Dialog open={showUnifyDialog} onOpenChange={setShowUnifyDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione qual cadastro será mantido como principal. Os demais serão marcados como inativos.
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedGroup && (
            <div className="space-y-4">
              <div className="space-y-3">
                <Label className="text-sm font-medium">Cadastro Principal</Label>
                <RadioGroup value={primaryCitizenId} onValueChange={setPrimaryCitizenId}>
                  {getCitizensInGroup(selectedGroup).map((citizen: any) => (
                    <div key={citizen.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                      <RadioGroupItem value={citizen.id} id={citizen.id} />
                      <Label htmlFor={citizen.id} className="flex-1 cursor-pointer">
                        <p className="font-medium">{citizen.no_cidadao}</p>
                        <p className="text-xs text-gray-500">
                          CPF: {citizen.nu_cpf || '-'} | CNS: {citizen.nu_cns || '-'}
                        </p>
                      </Label>
                    </div>
                  ))}
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button variant="outline" onClick={() => setShowUnifyDialog(false)}>
                  Cancelar
                </Button>
                <Button 
                  onClick={handleUnify}
