...
import { Skull, Search, Loader2, CheckCircle2, Eye, AlertCircle } from 'lucide-react';
...
export default function DeathsList() {
  ...
  const { data: citizens = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const all = await trpc.CitizenRecord.filter({ st_obito: 1 }, '-dt_obito', 1000);
      return all;
    }
  });
  ...
  const verifyMutation = useMutation({
    mutationFn: (citizenId: string) => trpc.CitizenRecord.update(citizenId, { 
      verified: true,
      verified_at: new Date().toISOString()
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['citizensWithDeath'] });
  ...
  const unverifiedCount = citizens.filter((c: any) => !c.verified).length;
      ...
      <Card className="shadow-lg border-0 bg-white/90">
        <CardHeader className="border-b bg-gray-50">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            Lista de Óbitos
            <Badge className="ml-2 bg-gray-200 text-gray-700">{filteredCitizens.length}</Badge>
          </CardTitle>
        </CardHeader>
            ...
            <>
                    ...
                    <AnimatePresence>
                      {paginatedCitizens.map((citizen: any, idx: number) => (
                        ...
                        >
                          ...
                          <TableCell className="font-mono text-sm">{citizen.nu_cpf || '-'}</TableCell>
                          <TableCell className="font-mono text-sm">{citizen.nu_cns || '-'}</TableCell>
                          <TableCell className="font-medium text-red-600">
                            {citizen.dt_obito 
                              ? new Date(citizen.dt_obito).toLocaleDateString('pt-BR') 
                              : '-'}
                          </TableCell>
                          <TableCell>
                            {citizen.verified ? (
                          ...
                          <TableCell className="text-center">
                            {!citizen.verified ? (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => verifyMutation.mutate(citizen.id)}
              ...
              {filteredCitizens.length === 0 && (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-lg font-medium">Nenhum registro de óbito encontrado</p>
                </div>
              )}
            </>
          )}
