...
export default function MissingCPFList() {
  ...
  const { data: citizens = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return all.filter(c => !c.nu_cpf || c.nu_cpf.trim() === "");
    },
  });

  // Create pending edit
  const createPendingEdit = useMutation({
    mutationFn: data => trpc.PendingEdit.create(data),
    onSuccess: () => {
