...
export default function ReportPreview({ config }) {
      ...
      try {
        ...
        if (config.query_config?.filters) {
          result = result.filter(item => {
            return config.query_config.filters.every(filter => {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const fields =
      config.query_config?.fields || Object.keys(data[0] || {}).slice(0, 5);
    ...
    const tableData = data
      .slice(0, 100)
      .map(item => fields.map(field => String(item[field] || "-")));
    ...
    doc.save(
      `${config.name || "relatorio"}-${new Date().toISOString().split("T")[0]}.pdf`
    );
