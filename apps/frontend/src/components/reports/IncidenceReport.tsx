...
export interface IncidenceReportProps {
  citizens: any[];
  areas: any[];
  visits: any[];
...
export default function IncidenceReport({ citizens, areas, visits, onExportCSV, onExportPDF }: IncidenceReportProps) {
  const [groupBy, setGroupBy] = useState<string>('condition');
  ...
  const incidenceData = useMemo(() => {
    if (groupBy === 'condition') {
      const conditionCounts: Record<string, any> = {};
    ...
    if (groupBy === 'microarea') {
      const microareaData: Record<string, any> = {};
      ...
      filteredCitizens.forEach(citizen => {
        ...
        (citizen.conditions || []).forEach((cond: string) => {
