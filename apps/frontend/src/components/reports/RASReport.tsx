...
export interface RASReportProps {
  visits?: any[];
  professionals?: any[];
  teams?: any[];
}

export default function RASReport({ visits = [], professionals = [], teams = [] }: RASReportProps) {
  ...
  const stats = useMemo(() => ({
    ...
    byTurno: rasData.reduce((acc: Record<string, number>, r) => {
              ...
              <Select value={filters.professional} onValueChange={(v) => setFilters({ ...filters, professional: v })}>
                ...
                <SelectContent>
                  ...
                  {uniqueProfessionals.map(prof => (
                    <SelectItem key={String(prof)} value={String(prof)}>{String(prof)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
