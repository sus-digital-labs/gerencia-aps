...
export default function BPAReport({ visits = [], professionals = [] }) {
  ...
  // Consolida somente registros com contrato mínimo completo.
  const bpaData = useMemo(() => {
    const grouped = new Map();
    visits
      .filter(visit => visit.procedure_code && visit.cbo && visit.cnes)
      .forEach(visit => {
        const key = `${visit.cnes}:${visit.cbo}:${visit.procedure_code}`;
        const current = grouped.get(key) || {
          code: visit.procedure_code,
          name: visit.procedure_name || visit.procedure_code,
          qty: 0,
          cbo: visit.cbo,
          cnes: visit.cnes,
          unitName: visit.unit_name || "ESTABELECIMENTO NÃO INFORMADO",
  ...
  const generateBPAFile = async () => {
    if (bpaData.length === 0) {
      toast.error("BPA não gerado: faltam procedimentos, CBO ou CNES válidos");
      return;
    }
    ...
    const header = [
      ...
      String(bpaData[0].cnes).padStart(7, "0"), // CNES validado na entrada
      String(bpaData[0].unitName).padEnd(60, " ").slice(0, 60),
    ...
    bpaData.forEach((proc, idx) => {
      const record = [
        "02", // Tipo registro
        competencia.year + competencia.month, // Competência
        String(proc.cnes).padStart(7, "0"), // CNES do registro
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/80">
                  Geração do arquivo consolidado para envio ao DATASUS
                </p>
              </div>
            </div>
            <Badge className="bg-white/20 text-white text-lg px-4 py-2">
              {MONTHS.find(m => m.value === competencia.month)?.label}/
              {competencia.year}
            </Badge>
          </div>
        </CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={v =>
                  setCompetencia({ ...competencia, month: v })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          {isGenerating && (
            <div className="mt-4">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Processando... {generationProgress}%
              </p>
            </div>
          )}
        </CardContent>
