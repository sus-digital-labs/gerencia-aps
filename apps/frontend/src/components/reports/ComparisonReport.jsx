...
export default function ComparisonReport({
  ...
  const [comparisonType, setComparisonType] = useState("microareas");
  ...
  const comparisonData = useMemo(() => {
    if (comparisonType === "microareas") {
      ...
      visits.forEach(v => {
        ...
        if (v.citizen_cns)
          microareaMap[v.microarea].citizens.add(v.citizen_cns);
        if (v.desfecho === "visita_realizada")
          microareaMap[v.microarea].completed++;
      });

      return Object.values(microareaMap)
        .map(ma => ({
          ...ma,
          citizens: ma.citizens.size,
          completionRate:
            ma.visits > 0 ? ((ma.completed / ma.visits) * 100).toFixed(1) : 0,
        }))
    ...
    if (comparisonType === "equipes") {
      return teams
        .map(team => {
          ...
          const avgResult =
            teamIndicators.length > 0
              ? teamIndicators.reduce(
                  (sum, i) => sum + (i.result_percentage || 0),
                  0
                ) / teamIndicators.length
              : 0;
        ...
        .sort(
          (a, b) => parseFloat(b.avgIndicator) - parseFloat(a.avgIndicator)
        );
    }
  ...
  const getMetricValue = item => {
    switch (metric) {
      ...
      case "citizens":
        return item.citizens || 0;
      case "completionRate":
        return parseFloat(item.completionRate) || 0;
      case "avgIndicator":
        return parseFloat(item.avgIndicator) || 0;
      default:
        return 0;
    }
  };
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="ml-auto flex gap-2">
              <Button
                ...
                size="sm"
                onClick={() =>
                  onExportCSV(comparisonData, `comparativo_${comparisonType}`)
                }
              >
                  ...
                  {comparisonType === "microareas" ? (
                    ...
                    <>
                      ...
                      <TableCell className="text-right">
                        <span
                          className={
                            parseFloat(item.avgIndicator) >= 50
                              ? "text-green-600"
                              : "text-orange-600"
                          }
                        >
