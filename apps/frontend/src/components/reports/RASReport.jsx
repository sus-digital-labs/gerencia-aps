...
export default function RASReport({
  visits = [],
  professionals = [],
  teams = [],
}) {
  ...
  const rasData = useMemo(() => {
    let data = visits.map((visit, idx) => ({
      ...
      turno: visit.visit_time
        ? parseInt(visit.visit_time.split(":")[0]) < 12
          ? "Manhã"
          : parseInt(visit.visit_time.split(":")[0]) < 18
            ? "Tarde"
            : "Noite"
        : "Manhã",
    }));
  ...
  const uniqueProfessionals = [
    ...new Set(visits.map(v => v.acs_name).filter(Boolean)),
  ];
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/80">
                  Atendimentos simplificados da Coleta de Dados Simplificada
                  (CDS)
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={v => setFilters({ ...filters, professional: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-4 border-t">
              <p className="text-sm text-gray-500">
                ...
                {Math.min(page * pageSize, rasData.length)} de {rasData.length}
              </p>
