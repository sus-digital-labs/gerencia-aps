...
export default function IncidenceReport({
    ...
    if (groupBy === "microarea") {
      ...
      return Object.values(microareaData)
        .map(ma => ({
          ...
          total: ma.total,
          ...Object.fromEntries(
            Object.entries(ma.conditions).map(([k, v]) => [
              conditionLabels[k] || k,
              v,
            ])
          ),
  ...
  const microareas = [
    ...new Set(citizens.map(c => c.microarea).filter(Boolean)),
  ];

  // Summary stats
  const totalWithConditions = filteredCitizens.filter(
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="ml-auto flex gap-2">
              <Button
                ...
                size="sm"
                onClick={() =>
                  onExportCSV(incidenceData, `incidencia_${groupBy}`)
                }
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-white/70">
                    {filteredCitizens.length > 0
                      ? (
                          (totalWithConditions / filteredCitizens.length) *
                          100
                        ).toFixed(1)
                      : 0}
                    %
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {mostCommonCondition?.label || "-"}
                  </p>
                  <p className="text-sm text-white/70">
                    {mostCommonCondition?.count || 0} casos
                  </p>
                </div>
              </div>
            </CardContent>
