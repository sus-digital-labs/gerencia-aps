...
import { 
  BarChart3, Download, Filter, Users, Building2, Calendar, FileSpreadsheet
} from 'lucide-react';
...
export default function ProductionReport({ professionals = [], teams = [], units = [], visits = [], onExport }: ProductionReportProps) {
  ...
  // Generate mock production data based on visits
  const productionData = useMemo(() => {
    const data: Record<string, any> = {};
    
    // Group by selection
    const groups = filters.groupBy === 'profissional' 
    ...
    groups.forEach((group: string) => {
      if (!data[group]) {
        ...
        PROCEDURES.forEach(proc => {
          // Generate random production numbers for demo
          data[group][proc.id] = Math.floor(Math.random() * 100) + 10;
        });
      }
    });
  ...
  const totals = useMemo(() => {
    const result: Record<string, any> = { name: 'TOTAL' };
    PROCEDURES.forEach(proc => {
      result[proc.id] = productionData.reduce((sum, row) => sum + (row[proc.id] || 0), 0);
    });
    result.total = PROCEDURES.reduce((sum, proc) => sum + (result[proc.id] || 0), 0);
    return result;
  }, [productionData]);
  ...
  const handleExportCSV = () => {
    const headers = ['Nome', ...PROCEDURES.filter(p => filters.selectedProcedures.includes(p.id)).map(p => p.name), 'Total'];
    const rows = productionData.map(row => [
      row.name,
      ...PROCEDURES.filter(p => filters.selectedProcedures.includes(p.id)).map(p => row[p.id] || 0),
      PROCEDURES.filter(p => filters.selectedProcedures.includes(p.id)).reduce((sum, p) => sum + (row[p.id] || 0), 0)
    ]);
    
    const csv = [headers, ...rows, [
      'TOTAL',
      ...PROCEDURES.filter(p => filters.selectedProcedures.includes(p.id)).map(p => totals[p.id]),
      totals.total
    ]].map(r => r.join(',')).join('\n');
  ...
  const selectedProcedures = PROCEDURES.filter(p => filters.selectedProcedures.includes(p.id));
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-end">
              <Button onClick={handleExportCSV} className="w-full gap-2 bg-emerald-600 hover:bg-emerald-700">
          ...
          <div className="pt-4 border-t">
            <Label className="text-sm font-medium mb-3 block">Procedimentos</Label>
      ...
      <Card className="shadow-lg border-0 bg-white/90">
        <CardHeader>
          <CardTitle className="text-lg">Produção por {filters.groupBy === 'profissional' ? 'Profissional' : filters.groupBy === 'equipe' ? 'Equipe' : 'Unidade'}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="h-80">
              ...
              <TableHeader>
                <TableRow className="bg-gray-50">
                  <TableHead className="font-bold sticky left-0 bg-gray-50">
                    {filters.groupBy === 'profissional' ? 'Profissional' : filters.groupBy === 'equipe' ? 'Equipe' : 'Unidade'}
                  </TableHead>
                  {selectedProcedures.map(proc => (
                    <TableHead key={proc.id} className="text-center whitespace-nowrap">
                      {proc.name}
                    </TableHead>
                  ))}
                  <TableHead className="text-center font-bold bg-blue-50">Total</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {productionData.map((row, idx) => (
                  <TableRow key={idx} className="hover:bg-gray-50">
                    <TableCell className="font-medium sticky left-0 bg-white">{row.name}</TableCell>
                    {selectedProcedures.map(proc => (
                    ...
                    <TableCell className="text-center font-bold bg-blue-50">
                      {selectedProcedures.reduce((sum, proc) => sum + (row[proc.id] || 0), 0)}
                    </TableCell>
                  </TableRow>
                ))}
                {/* Total Row */}
                <TableRow className="bg-slate-100 font-bold">
                  <TableCell className="sticky left-0 bg-slate-100">TOTAL</TableCell>
                  {selectedProcedures.map(proc => (
                    <TableCell key={proc.id} className="text-center">
                      {totals[proc.id]}
                    </TableCell>
                  ))}
                  <TableCell className="text-center bg-blue-200">{totals.total}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
