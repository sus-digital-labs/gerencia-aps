...
import { 
  Map, Key, CheckCircle, AlertTriangle, Eye, EyeOff, 
  Globe, Satellite, Navigation, Info, Save, RefreshCw
} from 'lucide-react';
import { toast } from 'sonner';

const MAP_TILE_OPTIONS = {
  free: [
    { value: 'carto-dark', label: 'CARTO Dark (Padrão)', description: 'Mapa escuro moderno, ideal para dashboards', preview: '🌑' },
    { value: 'carto-light', label: 'CARTO Light', description: 'Mapa claro e limpo', preview: '🌕' },
    { value: 'osm', label: 'OpenStreetMap', description: 'Mapa colaborativo com detalhes locais', preview: '🗺️' },
    { value: 'esri-topo', label: 'ESRI Topográfico', description: 'Mapa topográfico com relevo', preview: '⛰️' },
  ],
  google: [
    { value: 'google-roadmap', label: 'Google Roadmap', description: 'Mapa de ruas padrão do Google', preview: '🛣️' },
    { value: 'google-satellite', label: 'Google Satélite', description: 'Imagens de satélite de alta resolução', preview: '🛰️' },
    { value: 'google-hybrid', label: 'Google Híbrido', description: 'Satélite com rótulos de ruas', preview: '🌍' },
    { value: 'google-terrain', label: 'Google Terreno', description: 'Mapa topográfico com relevo', preview: '🏔️' },
  ],
};
...
export default function MapIntegrationConfig() {
  ...
  const { data: mapConfig, isLoading, refetch } = trpc.remapeamento.getMapConfig.useQuery();
  ...
  const validateGoogleKey = async () => {
    ...
    try {
      ...
      if (text.includes('InvalidKeyMapError') || text.includes('MissingKeyMapError')) {
        setKeyValid(false);
  ...
  const currentTileOptions = MAP_TILE_OPTIONS[mapProvider] || MAP_TILE_OPTIONS.free;
  const selectedTile = currentTileOptions.find((t: any) => t.value === mapTileStyle) || currentTileOptions[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-bold text-slate-800">Integração de Mapas</h3>
          <p className="text-sm text-slate-500">Configure o provedor de mapas usado em todo o sistema</p>
        </div>
        {isDirty && (
          <Badge variant="outline" className="ml-auto border-amber-400 text-amber-600 bg-amber-50">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <Badge className="bg-green-100 text-green-700 text-xs">Sem custo</Badge>
            </div>
          </div>
          <p className="text-sm text-slate-600">
            OpenStreetMap, CARTO e ESRI. Sem necessidade de API Key. 
            Ideal para uso interno e desenvolvimento.
          </p>
          <div className="mt-3 flex flex-wrap gap-1">
            {MAP_TILE_OPTIONS.free.map((t: any) => (
              <span key={t.value} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">
                {t.preview} {t.label}
              </span>
            ))}
          </div>
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <Badge className="bg-amber-100 text-amber-700 text-xs">Requer API Key</Badge>
            </div>
          </div>
          <p className="text-sm text-slate-600">
            Mapas do Google com satélite, Street View e Places. 
            Melhor qualidade de imagens e dados de localização.
          </p>
          <div className="mt-3 flex flex-wrap gap-1">
            {MAP_TILE_OPTIONS.google.map((t: any) => (
              <span key={t.value} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">
                {t.preview} {t.label}
              </span>
            ))}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Input
                  ...
                  placeholder="AIzaSy..."
                  value={googleApiKey}
                  onChange={(e: any) => { setGoogleApiKey(e.target.value); setIsDirty(true); setKeyValid(null); }}
                  className={`pr-10 font-mono text-sm ${
                    keyValid === true ? 'border-green-400 bg-green-50' :
                    keyValid === false ? 'border-red-400 bg-red-50' : ''
                  }`}
                />
                <button
                  type="button"
            ...
            {keyValid === false && (
              <Alert className="border-red-300 bg-red-50">
                ...
                <AlertDescription className="text-red-700 text-sm">
                  API Key inválida. Verifique se a key está correta e se as APIs necessárias estão habilitadas.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
      ...
      <div className="space-y-3">
        <Label className="text-sm font-semibold text-slate-700">Estilo do Mapa</Label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {currentTileOptions.map((tile) => (
            <button
              key={tile.value}
              onClick={() => { setMapTileStyle(tile.value); setIsDirty(true); }}
            ...
            >
              ...
              <p className="text-xs font-semibold text-slate-700 leading-tight">{tile.label}</p>
              <p className="text-xs text-slate-500 mt-0.5 leading-tight">{tile.description}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Label className="text-sm font-semibold text-slate-700">Coordenadas Padrão do Município</Label>
          <div className="group relative">
            ...
            <div className="absolute left-6 top-0 w-64 bg-slate-800 text-white text-xs rounded-lg p-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
              Centro do mapa ao abrir o módulo de Território. Padrão: Barra do Choça - BA.
            </div>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <Input
              ...
              step="0.0001"
              value={latCentro}
              onChange={(e: any) => { setLatCentro(e.target.value); setIsDirty(true); }}
          ...
          <div>
            ...
            <Input
              ...
              step="0.0001"
              value={lngCentro}
              onChange={(e: any) => { setLngCentro(e.target.value); setIsDirty(true); }}
              className="text-sm font-mono"
              placeholder="-40.5736"
            />
          </div>
          <div>
            <Label className="text-xs text-slate-500 mb-1 block">Zoom Inicial (1-20)</Label>
            <Input
              type="number"
              min="1"
              max="20"
              value={zoomInicial}
              onChange={(e: any) => { setZoomInicial(e.target.value); setIsDirty(true); }}
              className="text-sm"
              placeholder="13"
            />
          </div>
        ...
        <p className="text-xs text-slate-400">
          💡 Dica: Barra do Choça - BA: lat -14.8619, lng -40.5736, zoom 13
        </p>
      </div>
      ...
      <Card className="bg-slate-50 border-slate-200">
        <CardContent className="p-4">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Configuração Atual</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-white rounded-lg p-3 border border-slate-200">
              ...
              <p className="font-semibold text-slate-800">
                {mapProvider === 'google'
                  ? googleApiKey ? '✅ Configurada' : '⚠️ Não configurada'
                  : '—'
                }
              </p>
            </div>
