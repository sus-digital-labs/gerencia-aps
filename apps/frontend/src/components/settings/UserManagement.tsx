                ...
                <TableRow key={user.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <Badge className={ROLE_COLORS[user.role] || ROLE_COLORS.user}>
                      {ROLES.find((r: any) => r.value === user.role)?.label || user.role || 'Usuário'}
                    </Badge>
                  </TableCell>
                  <TableCell>{units.find((u: any) => u.id === user.unit_id)?.name || '-'}</TableCell>
                  <TableCell>{teams.find((t: any) => t.id === user.team_id)?.name || '-'}</TableCell>
                  <TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Papel</Label>
              <Input value={ROLES.find((r: any) => r.value === formData.role)?.label || formData.role} disabled className="bg-gray-50" />
            </div>
            <div className="space-y-2">
              <Label>Unidade de Saúde</Label>
              <Select value={formData.unit_id} onValueChange={(v: any) => setFormData({ ...formData, unit_id: v })}>
            ...
            <div className="space-y-2">
              <Label>Equipe</Label>
              <Select value={formData.team_id} onValueChange={(v: any) => setFormData({ ...formData, team_id: v })}>
                <SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger>
                <SelectContent>
                  <SelectItem value={null}>Nenhuma</SelectItem>
