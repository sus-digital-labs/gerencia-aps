...
const RULE_TYPES = {
  cv_risk_no_visit: {
    label: "Paciente Alto Risco CV sem Visita",
    icon: "❤️",
    color: "bg-red-100 text-red-700",
  },
  data_inconsistency: {
    label: "Inconsistência Crítica em Dados",
    icon: "⚠️",
...
export default function AlertRulesManager() {
  ...
  const createMutation = useMutation({
    mutationFn: data => trpc.AlertRule.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => trpc.AlertRule.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["alertRules"]);
      setIsDialogOpen(false);
      setEditingRule(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: id => trpc.AlertRule.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries(["alertRules"]);
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: ({ id, active }) => trpc.AlertRule.update(id, { active }),
    onSuccess: () => {
      queryClient.invalidateQueries(["alertRules"]);
        ...
        <div className="space-y-3">
          {rules.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm">
                Crie regras para automatizar alertas no sistema
              </p>
            </div>
          ) : (
            rules.map(rule => {
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              confirm("Deseja realmente excluir esta regra?")
                            ) {
