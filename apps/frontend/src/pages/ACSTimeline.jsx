...
export default function ACSTimeline() {
  ...
  const { data: acsInfo } = useQuery({
    ...
    queryFn: () =>
      acsId
        ? trpc.CommunityHealthAgent.filter({ id: acsId })
        : Promise.resolve([]),
    enabled: !!acsId,
  });

  // Fetch visits for the ACS on selected date
  const { data: visits = [], isLoading } = useQuery({
    queryKey: ["acsVisits", acsId, selectedDate],
    queryFn: async () => {
      if (!acsId) return [];
      const allVisits = await trpc.HomeVisit.filter(
        { acs_id: acsId },
        "visit_time"
      );
      return allVisits.filter(v => v.visit_date === selectedDate);
    },
    enabled: !!acsId,
  });
  ...
  const getInconsistencies = visit => {
    const issues = [];
    if (!visit.citizen_cns)
      issues.push({ field: "citizen_cns", label: "CNS não informado" });
    if (!visit.latitude || !visit.longitude)
      issues.push({
        field: "coordinates",
        label: "Coordenadas não registradas",
      });
    return issues;
  };
  ...
  const handleSaveEdit = async () => {
    // In a real app, this would create a pending edit for approval
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-amber-600">
                  {new Set(sortedVisits.map(v => v.family_id)).size}
                </p>
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-blue-600">
                  {
                    sortedVisits.filter(v => v.desfecho === "visita_realizada")
                      .length
                  }
                </p>
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-red-600">
                  {
                    sortedVisits.filter(v => getInconsistencies(v).length > 0)
                      .length
                  }
                </p>
                        ...
                        {visit.conditions_found?.length > 0 && (
                          <div className="pt-2 border-t">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {visit.conditions_found.map((cond, i) => {
                                const Icon =
                                  procedureIcons[cond] ||
                                  procedureIcons.default;
                                return (
                                  <Badge
                                    key={i}
                                    variant="outline"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button onClick={handleSaveEdit}>Enviar para Aprovação</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
