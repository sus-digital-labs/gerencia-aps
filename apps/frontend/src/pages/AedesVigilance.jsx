...
export default function AedesVigilance() {
  ...
  const filteredFocuses = useMemo(() => {
    ...
    if (filters.startDate)
      data = data.filter(f => f.visit_date >= filters.startDate);
    if (filters.endDate)
      data = data.filter(f => f.visit_date <= filters.endDate);
    if (filters.neighborhood !== "all")
      data = data.filter(f => f.neighborhood === filters.neighborhood);
    if (filters.focusType !== "all")
      data = data.filter(f => f.focus_type === filters.focusType);
    if (search) {
  ...
  const filteredCases = useMemo(() => {
    let data = cases;
    if (filters.startDate)
      data = data.filter(c => c.notification_date >= filters.startDate);
    if (filters.endDate)
      data = data.filter(c => c.notification_date <= filters.endDate);
    if (filters.neighborhood !== "all")
      data = data.filter(c => c.neighborhood === filters.neighborhood);
    return data;
  }, [cases, filters]);
  ...
  const stats = useMemo(() => {
    ...
    const focosEliminados = filteredFocuses.filter(
      f => f.focus_eliminated
    ).length;
    ...
    const iip =
      imoveisVisitados > 0
        ? ((imoveisComFoco / imoveisVisitados) * 100).toFixed(1)
        : 0;

    const dengueSuspeitos = filteredCases.filter(
      c => c.disease_type === "dengue" && c.status === "suspeito"
    ).length;
    const dengueConfirmados = filteredCases.filter(
      c => c.disease_type === "dengue" && c.status === "confirmado"
    ).length;
    const zikaCases = filteredCases.filter(
      c => c.disease_type === "zika"
    ).length;
    const chikungunyaCases = filteredCases.filter(
      c => c.disease_type === "chikungunya"
    ).length;
  ...
  const casesByDisease = [
    {
      name: "Dengue",
      suspeitos: stats.dengueSuspeitos,
      confirmados: stats.dengueConfirmados,
    },
    {
      name: "Zika",
      suspeitos: filteredCases.filter(
        c => c.disease_type === "zika" && c.status === "suspeito"
      ).length,
      confirmados: filteredCases.filter(
        c => c.disease_type === "zika" && c.status === "confirmado"
      ).length,
    },
    {
      name: "Chikungunya",
      suspeitos: filteredCases.filter(
        c => c.disease_type === "chikungunya" && c.status === "suspeito"
      ).length,
      confirmados: filteredCases.filter(
        c => c.disease_type === "chikungunya" && c.status === "confirmado"
      ).length,
    },
  ];
  ...
  const neighborhoods = [
    ...new Set(
      [
        ...focuses.map(f => f.neighborhood),
        ...cases.map(c => c.neighborhood),
      ].filter(Boolean)
    ),
  ];
  ...
  const mapCenter =
    mapData.length > 0 && mapData[0].latitude
      ? [mapData[0].latitude, mapData[0].longitude]
      : defaultCenter;

  // Pagination
  const paginatedFocuses = filteredFocuses.slice(
    (page - 1) * pageSize,
    page * pageSize
  );
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight">
                  Vigilância Aedes aegypti
                </h1>
                <p className="text-white/70">
                  Monitoramento de focos e casos de Dengue, Zika e Chikungunya
                </p>
              </div>
            </div>
            <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={v =>
                    setFilters({ ...filters, neighborhood: v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={v => setFilters({ ...filters, focusType: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">Todos</SelectItem>
                    {Object.entries(FOCUS_TYPE_LABELS).map(([k, v]) => (
            ...
            <CardHeader className="border-b bg-gradient-to-r from-red-500 to-orange-500 text-white">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant={mapView === "focos" ? "secondary" : "ghost"}
                    onClick={() => setMapView("focos")}
                    className={
                      mapView === "focos" ? "" : "text-white hover:bg-white/20"
                    }
                  >
                    Focos
                  </Button>
                  <Button
                    size="sm"
                    variant={mapView === "casos" ? "secondary" : "ghost"}
                    onClick={() => setMapView("casos")}
                    className={
                      mapView === "casos" ? "" : "text-white hover:bg-white/20"
                    }
                  >
                    Casos
                  </Button>
                ...
                {mapData.map((item, idx) => (
                  <CircleMarker
                    ...
                    fillColor={
                      mapView === "focos"
                        ? "#ef4444"
                        : DISEASE_COLORS[item.disease_type] || "#ef4444"
                    }
                    color="#fff"
                    weight={2}
                    opacity={1}
                        ...
                        {mapView === "focos" ? (
                          <>
                            <p className="font-bold">
                              {FOCUS_TYPE_LABELS[item.focus_type]}
                            </p>
                            <p>{item.address}</p>
              ...
              <TableBody>
                {paginatedFocuses.map((focus, idx) => (
                  <TableRow key={idx} className="hover:bg-gray-50">
                    <TableCell>
                      {focus.visit_date
                        ? new Date(focus.visit_date).toLocaleDateString("pt-BR")
                        : "-"}
                    </TableCell>
                    ...
                    <TableCell>
                      {PROPERTY_TYPE_LABELS[focus.property_type] ||
                        focus.property_type}
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline">
                        {FOCUS_TYPE_LABELS[focus.focus_type] ||
                          focus.focus_type}
                      </Badge>
                    </TableCell>
                    <TableCell>
