...
function geojsonToLeaflet(geojsonStr) {
  try {
...
function isMultiPolygon(geojsonStr) {
  try {
    const geojson = JSON.parse(geojsonStr);
    const geom = geojson.geometry || geojson;
...
const MUNICIPALITY_CENTER = runtimeConfig.municipality.center;

// Gerar KML a partir das microáreas
function gerarKML(microareas) {
  const placemarks = microareas
    .map(ma => {
      let coordStr = "";
      try {
...
function extrairCoordsPoligono(polygon) {
  const coordsText =
  ...
  return coordsText
    ...
    .filter(
      ([lng, lat]) => !isNaN(lng) && !isNaN(lat) && (lng !== 0 || lat !== 0)
    );
}
...
function parsearKML(kmlText) {
  ...
  placemarks.forEach(pm => {
    const nomeCompleto =
      pm.querySelector("name")?.textContent?.trim() || "Área importada";
    ...
    const styleUrl =
      pm.querySelector("styleUrl")?.textContent?.replace("#", "") || "";
    ...
    todasCoords.forEach(c => grupos[chave].coordsList.push(c));
  });
  ...
  return Object.values(grupos).map(({ nomeArea, nomeAcs, cor, coordsList }) => {
    ...
    const coordsPrincipal = coordsList.reduce(
...
const statusCor = s =>
  s === "excesso" ? "#EF4444" : s === "baixa_cobertura" ? "#F59E0B" : "#10B981";
const statusLabel = s =>
  s === "excesso"
    ? "Excesso"
    : s === "baixa_cobertura"
...
export default function MicroareaMapping() {
  ...
  const relatorioQuery = trpc.microareas.relatorioCobertura.useQuery(
    undefined,
  ...
  const gerarMutation = trpc.microareas.gerarMicroareas.useMutation({
    onSuccess: res => {
      ...
      msg(
        `${res.microareasGeradas} microáreas geradas!${res.alertas?.length > 0 ? ` ⚠️ ${res.alertas.length} alertas.` : ""}`,
        res.alertas?.length > 0 ? "warning" : "success"
      );
      setActiveTab("mapa");
    },
    onError: e => msg("Erro ao gerar: " + e.message, "error"),
  });

  const salvarMutation = trpc.microareas.salvarMicroarea.useMutation({
    onSuccess: () => utils.microareas.listarMicroareas.invalidate(),
    onError: e => msg("Erro ao salvar: " + e.message, "error"),
  });
  ...
  async function gerarMicroareasHandler() {
    gerarMutation.mutate({
      ...
      usarPerfilAcs: true,
      coordenadasMunicipio: MUNICIPALITY_CENTER,
    });
  }

  async function exportarKML() {
    if (microareas.length === 0) {
      msg("Nenhuma microárea para exportar.", "warning");
      return;
    }
    try {
  ...
  async function importarKML(event) {
    ...
    try {
      ...
      if (isKmz) {
        ...
        if (!kmlFile) {
          msg("Nenhum arquivo .kml encontrado dentro do KMZ.", "error");
          return;
        }
      ...
      if (areas.length === 0) {
        msg("Nenhum polígono encontrado no arquivo.", "error");
        return;
      }
      setKmlImportado(areas);
      msg(
        `${areas.length} área(s) importada(s) do ${isKmz ? "KMZ" : "KML"}. Clique em "Aplicar" para salvar.`
      );
    } catch (err) {
  ...
  const mapCenter = [MUNICIPALITY_CENTER.lat, MUNICIPALITY_CENTER.lng];
    ...
    >
      ...
      >
        <div>
          ...
          >
            🗺️ Mapeamento de Microáreas ACS
          </h1>
          <p style={{ margin: "2px 0 0", fontSize: "12px", color: "#64748b" }}>
            Distribuição territorial inteligente — Portaria MS 2.436/2017
          </p>
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <input
            ...
            accept=".kmz,.kml"
            style={{ display: "none" }}
            onChange={importarKML}
          />
          <button onClick={exportarKML} style={s.btn("#0ea5e9")}>
            ⬇️ Exportar KMZ
          </button>
        </div>
      </div>
            ...
            color:
              mensagem.tipo === "error"
                ...
                : mensagem.tipo === "warning"
                  ? "#92400E"
                  : "#065F46",
            border: `1px solid ${mensagem.tipo === "error" ? "#FCA5A5" : mensagem.tipo === "warning" ? "#FCD34D" : "#6EE7B7"}`,
      ...
      {kmlImportado && (
        ...
        >
          <span style={{ fontSize: "14px", color: "#5B21B6" }}>
            📂 <strong>{kmlImportado.length} área(s)</strong> carregada(s) do
            arquivo KMZ/KML.
          </span>
          <button onClick={aplicarKML} style={s.btn("#7C3AED")}>
            ✅ Aplicar
          </button>
          <button
            onClick={() => setKmlImportado(null)}
            style={{
              ...s.btn("transparent", "#7C3AED"),
      ...
      {levantamento && (
        ...
        >
          {[
            {
              label: "Cidadãos",
              value: levantamento.totalCidadaos?.toLocaleString("pt-BR"),
              icon: "👥",
              color: "#3B82F6",
            },
            {
              label: "Famílias",
              value: levantamento.totalFamilias?.toLocaleString("pt-BR"),
              icon: "🏠",
              color: "#10B981",
            },
            {
              label: "ACS",
              value: levantamento.totalAcs,
              icon: "🧑‍⚕️",
              color: "#8B5CF6",
            },
            {
              label: "Recomendadas",
              value: levantamento.numeroDeMicroareasRecomendado,
              icon: "🗺️",
              color: "#F59E0B",
            },
            {
            ...
            {
              label: "Limite MS (Fam.)",
              value: levantamento.limiteMsFamilias,
              icon: "⚖️",
              color: "#EF4444",
            },
          ].map((c, i) => (
            ...
            >
              ...
              <div
                style={{ fontSize: "18px", fontWeight: 700, color: c.color }}
              >
                {c.value}
              </div>
          ...
          {levantamento.fonte === "mock" && (
            ...
            >
              <span style={{ fontSize: "11px", color: "#92400E" }}>
                ⚠️ Dados de demonstração. Conecte ao PEC para dados reais.
              </span>
            </div>
          )}
        </div>
          ...
          >
            <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
              ...
              >
                ...
                {microareas.map((ma, i) => {
                  const coordsLists = geojsonToLeaflet(
                    ma.geojsonPoligono || ""
                  );
                    ...
                    <Popup>
                      <div style={{ minWidth: "180px" }}>
                        ...
                        <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
                          <div>
                            👤 <b>ACS:</b> {ma.acsNome || "Não atribuído"}
                          </div>
                          <div>
                            🏠 <b>Famílias:</b> {ma.totalFamilias || 0}
                          </div>
                          <div>
                            👥 <b>Cidadãos:</b> {ma.totalCidadaos || 0}
                          </div>
                          <div>
                            📊 <b>Status:</b>{" "}
                            <span style={{ color: statusCor(ma.status) }}>
                              {statusLabel(ma.status)}
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  return coordsLists.map((coords, j) => {
                    ...
                    return (
                      <Polygon
                        ...
                        pathOptions={{
                          color: cor,
                          fillColor: cor,
                          fillOpacity: selecionada?.id === ma.id ? 0.5 : 0.25,
                          weight: selecionada?.id === ma.id ? 3 : 2,
                        }}
                ...
                {kmlImportado &&
                  kmlImportado.map((area, i) => {
                    const coordsLists = geojsonToLeaflet(
                      area.geojsonPoligono || ""
                    );
                    return coordsLists.map((coords, j) => {
                      if (!coords || coords.length < 3) return null;
                      return (
                        <Polygon
                          ...
                          positions={coords}
                          pathOptions={{
                            color: area.cor || "#7C3AED",
                            fillColor: area.cor || "#7C3AED",
                            fillOpacity: 0.3,
                            weight: 2,
                            dashArray: "5,5",
                          }}
                        >
                          <Popup>
                            <strong style={{ color: area.cor || "#7C3AED" }}>
                              {area.nome}
                            </strong>
                            {area.acsNome && (
                              <div
                            ...
                            <br />
                            <small>Aguardando aplicação</small>
                          </Popup>
                        </Polygon>
                      );
                    });
              ...
              {microareas.length > 0 && (
                ...
                >
                  ...
                  >
                    MICROÁREAS ({microareas.length})
                  </div>
                  {microareas.slice(0, 15).map((ma, i) => (
                    <div
                      key={i}
                      onClick={() => setSelecionada(ma)}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "6px",
                        ...
                        borderRadius: "4px",
                        background:
                          selecionada?.id === ma.id ? "#EFF6FF" : "transparent",
                      }}
                    >
                      <div
                        style={{
              ...
              {microareas.length === 0 && !loading && (
                ...
                >
                  ...
                  >
                    Use a aba "Gerar Automaticamente" para criar microáreas com
                    K-Means, ou importe um arquivo KML.
                  </p>
                  <button
              ...
              >
                ...
                >
                  <div
                    style={{
                      ...
                      height: "14px",
                      borderRadius: "3px",
                      background: selecionada.cor || "#3B82F6",
                    }}
                  />
                  <span
                    style={{
                      fontSize: "12px",
                      fontWeight: 600,
                      color: statusCor(selecionada.status),
                      background: `${statusCor(selecionada.status)}20`,
                      padding: "2px 8px",
                      borderRadius: "10px",
                    }}
                  >
                ...
                >
                  {[
                    {
                      label: "Famílias",
                      value: selecionada.totalFamilias || 0,
                      max: 450,
                      icon: "🏠",
                    },
                    {
                      label: "Cidadãos",
                      value: selecionada.totalCidadaos || 0,
                      max: 750,
                      icon: "👥",
                    },
                  ].map((stat, i) => (
                    ...
                    >
                      ...
                      <div
                        style={{
                          fontSize: "20px",
                          fontWeight: 700,
                          color: stat.value > stat.max ? "#EF4444" : "#1e293b",
                        }}
                      >
                        {stat.value}
                      </div>
                      ...
                      >
                        <div
                          style={{
                            ...
                            background:
                              stat.value > stat.max ? "#EF4444" : "#10B981",
                            borderRadius: "2px",
                          }}
                        />
                      </div>
                ...
                >
                  ...
                  >
                    {selecionada.acsNome || "Não atribuído"}
                  </div>
                </div>
        ...
        {activeTab === "gerar" && (
          <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
            <div style={{ maxWidth: "680px", margin: "0 auto" }}>
              ...
              >
                ⚡ Geração Automática de Microáreas
              </h2>
              <p
                style={{
                  color: "#64748b",
              ...
              >
                Algoritmo K-Means agrupa domicílios geograficamente respeitando
                os limites do Ministério da Saúde e o perfil de cada ACS.
              </p>
                  ...
                  >
                    {[
                      ...
                      { label: "ACS", value: levantamento.totalAcs },
                      {
                        label: "Média Cid./ACS",
                        value:
                          levantamento.mediaCidadaosPorAcs?.toLocaleString(
                            "pt-BR"
                          ),
                      },
                      {
                        label: "Média Fam./ACS",
                        value:
                          levantamento.mediaFamiliasPorAcs?.toLocaleString(
                            "pt-BR"
                          ),
                      },
                      {
                        label: "Microáreas Rec.",
                        value: levantamento.numeroDeMicroareasRecomendado,
                      },
                    ].map((item, i) => (
                      <div
              ...
              <div style={{ ...s.card, marginBottom: "16px" }}>
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={numMicroareas}
                    onChange={e => setNumMicroareas(e.target.value)}
                    min="1"
                    max="200"
                    placeholder={
                      levantamento?.numeroDeMicroareasRecomendado || "41"
                    }
                    style={{
                      width: "100%",
                      padding: "8px 12px",
                  ...
                  >
                    Recomendado:{" "}
                    {levantamento?.numeroDeMicroareasRecomendado || "—"}
                  </div>
                </div>
              </div>
              ...
              >
                ...
                >
                  ✅ Limites do Ministério da Saúde (Portaria 2.436/2017)
                </h3>
                <div
                  style={{
                    display: "grid",
              ...
              <button
                onClick={gerarMicroareasHandler}
                disabled={gerarMutation.isPending}
                style={{
                  width: "100%",
                  padding: "14px",
                  background: gerarMutation.isPending ? "#94a3b8" : "#3B82F6",
                  color: "white",
                  border: "none",
                  borderRadius: "10px",
                  cursor: gerarMutation.isPending ? "not-allowed" : "pointer",
                  fontSize: "15px",
                  fontWeight: 700,
                }}
              >
                {gerarMutation.isPending
                  ? "⏳ Gerando microáreas com K-Means..."
                  : `⚡ Gerar ${numMicroareas || levantamento?.numeroDeMicroareasRecomendado || "?"} Microáreas`}
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === "acs" && (
          <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
            <div style={{ maxWidth: "900px", margin: "0 auto" }}>
              ...
              >
                O sistema ajusta automaticamente as microáreas com base no
                perfil de cada ACS: idade, experiência e localização.
              </p>
              {acsList.length === 0 ? (
                <div
                  style={{
                ...
                >
                  ...
                  <div style={{ fontSize: "13px", marginTop: "8px" }}>
                    Conecte ao banco PEC para carregar os ACS reais.
                  </div>
                </div>
              ) : (
                <div
                    ...
                    <div key={acs.id || i} style={s.card}>
                      ...
                      >
                        {[
                          {
                            label: "Idade",
                            value: acs.idadeAcs ? `${acs.idadeAcs} anos` : "—",
                          },
                          {
                            label: "Experiência",
                            value: acs.anosExperiencia
                              ? `${acs.anosExperiencia} anos`
                              : "—",
                          },
                          {
                            label: "Cap. Famílias",
                            value: acs.capacidadeMaxFamilias || 450,
                            warn: (acs.capacidadeMaxFamilias || 450) < 400,
                          },
                          {
                            label: "Cap. Cidadãos",
                            value: acs.capacidadeMaxCidadaos || 750,
                            warn: (acs.capacidadeMaxCidadaos || 750) < 600,
                          },
                        ].map((item, j) => (
                          <div
                            key={j}
                      ...
                      {acs.idadeAcs >= 55 && (
                        ...
                        >
                          ⚠️ ACS sênior — capacidade reduzida
                        </div>
                      )}
                      {acs.anosExperiencia >= 10 && (
                        <div
                          style={{
                            marginTop: "6px",
                            fontSize: "11px",
                        ...
                        >
                          ⭐ Experiente — pode assumir áreas complexas
                        </div>
                      )}
                    </div>
                  ))}
          ...
          <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
            <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
              ...
              >
                ...
                >
                  📋 Lista de Microáreas ({microareas.length})
                </h2>
                <button onClick={exportarKML} style={s.btn("#0ea5e9")}>
                  ⬇️ Exportar todas KML
                </button>
              </div>
              {microareas.length === 0 ? (
                <div
                        ...
                        >
                          ...
                          <td
                            style={{
                              ...
                              fontSize: "13px",
                              color:
                                (ma.totalFamilias || 0) > 450
                                  ? "#EF4444"
                                  : "#374151",
                              fontWeight:
                                (ma.totalFamilias || 0) > 450 ? 700 : 400,
                            }}
                          >
                            {ma.totalFamilias || 0}
                          </td>
                          <td
                            style={{
                              ...
                              fontSize: "13px",
                              color:
                                (ma.totalCidadaos || 0) > 750
                                  ? "#EF4444"
                                  : "#374151",
                              fontWeight:
                                (ma.totalCidadaos || 0) > 750 ? 700 : 400,
                            }}
                          >
                            {ma.totalCidadaos || 0}
                          </td>
                            ...
                            >
                              ...
                              >
                                <div
                                  style={{
                                    height: "100%",
                                    width: `${Math.min(100, ((ma.totalFamilias || 0) / 450) * 100)}%`,
                                    background:
                                      (ma.totalFamilias || 0) > 450
                                        ? "#EF4444"
                                        : "#10B981",
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            <span
                              style={{
                                ...
                                fontWeight: 600,
                                color: statusCor(ma.status),
                                background: `${statusCor(ma.status)}20`,
                                padding: "2px 8px",
                                borderRadius: "10px",
                              }}
                            >
        ...
        {activeTab === "relatorio" && (
          <div style={{ flex: 1, padding: "24px", overflowY: "auto" }}>
            <div style={{ maxWidth: "900px", margin: "0 auto" }}>
              ...
              >
                📊 Relatório de Cobertura Territorial
              </h2>
              {relatorioQuery.isLoading ? (
                ...
                >
                  ...
                  <div style={{ fontSize: "13px", marginTop: "8px" }}>
                    Gere microáreas primeiro para ver o relatório.
                  </div>
                </div>
              ) : (
                <>
                  ...
                  >
                    {[
                      {
                        label: "Total Microáreas",
                        value: relatorio.resumo.totalMicroareas,
                        color: "#3B82F6",
                        icon: "🗺️",
                      },
                      {
                        label: "Dentro do Limite",
                        value: relatorio.resumo.microareasNormais,
                        color: "#10B981",
                        icon: "✅",
                      },
                      {
                        label: "Com Excesso",
                        value: relatorio.resumo.microareasExcesso,
                        color: "#EF4444",
                        icon: "⚠️",
                      },
                      {
                        label: "Baixa Cobertura",
                        value: relatorio.resumo.microareasBaixaCobertura,
                        color: "#F59E0B",
                        icon: "📉",
                      },
                    ].map((item, i) => (
                  ...
                  <div style={{ ...s.card, marginBottom: "16px" }}>
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          fontWeight: 700,
                          color:
                            relatorio.resumo.percentualCobertura >= 80
                              ? "#10B981"
                              : "#EF4444",
                        }}
                      >
                        {relatorio.resumo.percentualCobertura}%
                      </span>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${relatorio.resumo.percentualCobertura}%`,
                          background:
                            relatorio.resumo.percentualCobertura >= 80
                              ? "#10B981"
                              : "#F59E0B",
                  ...
                  {relatorio.alertas?.length > 0 && (
                    ...
                    >
                      ...
                      >
                        ⚠️ Alertas ({relatorio.alertas.length})
                      </h3>
                      {relatorio.alertas.map((alerta, i) => (
                        ...
                        >
                          ...
                          >
                            {alerta.tipo === "excesso" ? "EXCESSO" : "BAIXA"}
                          </span>
                          <span style={{ fontSize: "13px", color: "#374151" }}>
                            <strong>{alerta.microarea}:</strong>{" "}
                            {alerta.mensagem}
                          </span>
                        </div>
                      ))}
                                ...
                                >
                                  ...
                                  >
                                    <div
                                      style={{
                                        height: "100%",
                                        width: `${Math.min(100, acs.percentualLimiteFamilias)}%`,
                                        background:
                                          acs.percentualLimiteFamilias > 100
                                            ? "#EF4444"
                                            : acs.percentualLimiteFamilias > 80
                                              ? "#F59E0B"
                                              : "#10B981",
                                        borderRadius: "3px",
                                      }}
                              ...
                              <td style={{ padding: "8px 12px" }}>
                                <span
                                  style={{
                                    fontSize: "11px",
                                    fontWeight: 600,
                                    color: acs.dentroDoLimite
                                      ? "#10B981"
                                      : "#EF4444",
                                    background: acs.dentroDoLimite
                                      ? "#D1FAE5"
                                      : "#FEE2E2",
                                    padding: "2px 8px",
                                    borderRadius: "10px",
