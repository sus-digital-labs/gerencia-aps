...
export default function DataQuality() {
  ...
  const criticalIssues = issues.filter(
    i => i.severity === "critica" && i.status === "aberto"
  ).length;
  const activeCitizens = citizens.filter(c => c.st_ativo === 1);
  const withoutCPF = activeCitizens.filter(
    c => !c.nu_cpf || c.nu_cpf.trim() === ""
  ).length;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/70">
                  Central de gestão e correção de cadastros
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        >
          <TabsList className="bg-white/80 shadow-lg p-1 h-auto flex-wrap">
            ...
            <TabsTrigger
              value="obitos"
              className="gap-2 data-[state=active]:bg-gray-700 data-[state=active]:text-white"
            >
              <Skull className="w-4 h-4" />
              Óbitos
              {deathRecords > 0 && (
                <Badge className="ml-1 bg-gray-600 text-white">
                  {deathRecords}
                </Badge>
              )}
            </TabsTrigger>
