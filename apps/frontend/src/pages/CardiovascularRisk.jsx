...
export default function CardiovascularRiskPage() {
  ...
  const [carePlanDialog, setCarePlanDialog] = useState({
    open: false,
    patient: null,
  });
  ...
  // Fetch citizens for calculation
  const { data: citizens = [] } = useQuery({
    queryKey: ["citizenRecords"],
    queryFn: () => trpc.CitizenRecord.filter({}, "-created_date", 2000),
  });

  // Calculate risk for a citizen (Framingham-based simplified)
  const calculateRisk = citizen => {
    const systolicBP = Number(citizen.systolic_bp);
    const totalCholesterol = Number(citizen.total_cholesterol);
    const hdl = Number(citizen.hdl_cholesterol);
    if (![systolicBP, totalCholesterol, hdl].every(Number.isFinite))
      return null;
    ...
    const age =
      ...
      (citizen.dt_nascimento
        ? Math.floor(
            (new Date() - new Date(citizen.dt_nascimento)) / 31536000000
          )
        : 0);
    ...
    const isSmoker = citizen.is_smoker === true;
    const isDiabetic =
      citizen.is_diabetic === true || citizen.conditions?.includes("diabetes");
    const isHypertensive =
      citizen.is_hypertensive === true ||
      citizen.conditions?.includes("hipertensao");
  ...
  // Run risk calculation job
  const runCalculation = async () => {
    setIsCalculating(true);
    try {
      // Filter adults 30-74 years
      const eligibleCitizens = citizens
        .filter(c => {
          const age = c.dt_nascimento
            ? Math.floor((new Date() - new Date(c.dt_nascimento)) / 31536000000)
            : 0;
      ...
      queryClient.invalidateQueries(["cardiovascularRisk"]);
      toast.success(
        "Cálculo concluído somente para registros com critérios clínicos completos"
      );
    } catch (error) {
  ...
  const filteredData = useMemo(() => {
    let data = riskData;
    if (selectedRisk !== "all") {
      data = data.filter(r => r.risk_level === selectedRisk);
    }
    if (search) {
  ...
  const stats = useMemo(() => {
    ...
    const baixo = riskData.filter(r => r.risk_level === "baixo").length;
    const intermediario = riskData.filter(
      r => r.risk_level === "intermediario"
    ).length;
    const alto = riskData.filter(r => r.risk_level === "alto").length;
    const emAcompanhamento = riskData.filter(
      r => r.care_plan_status === "em_acompanhamento"
    ).length;
  ...
  const pieData = [
    { name: "Baixo Risco", value: stats.baixo, color: RISK_COLORS.baixo },
    {
      ...
      color: RISK_COLORS.intermediario,
    },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-red-50">
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/70">
                  Estratificação de risco baseada no Escore de Framingham
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="gap-2 bg-white text-rose-600 hover:bg-rose-50"
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-8">
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 cursor-pointer hover:bg-white/20"
              onClick={() => setSelectedRisk("baixo")}
            >
              <div className="flex items-center gap-3">
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 cursor-pointer hover:bg-white/20"
              onClick={() => setSelectedRisk("intermediario")}
            >
              <div className="flex items-center gap-3">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      onClick={data =>
                        setSelectedRisk(
                          data.name === "Baixo Risco"
                            ? "baixo"
                            : data.name === "Risco Intermediário"
                              ? "intermediario"
                              : "alto"
                        )
                      }
        ...
        <Card className="shadow-lg border-0 bg-white/90">
          <CardHeader className="border-b bg-gray-50">
            <CardTitle className="flex items-center gap-2">
              ...
              <Badge
                className={`ml-2 ${selectedRisk === "alto" ? "bg-red-100 text-red-700" : selectedRisk === "intermediario" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"}`}
              >
                ...
                <TableBody>
                  {paginatedData.map((patient, idx) => (
                    <TableRow key={idx} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <Badge
                          className={`${patient.risk_level === "alto" ? "bg-red-100 text-red-700" : patient.risk_level === "intermediario" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}
                        >
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={
                            patient.care_plan_status === "em_acompanhamento"
                      ...
                      <TableCell>
                        {patient.risk_level === "alto" &&
                          patient.care_plan_status === "pendente" && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() =>
                                setCarePlanDialog({ open: true, patient })
                              }
                            >
      ...
      {/* Care Plan Dialog */}
      <Dialog
        open={carePlanDialog.open}
        onOpenChange={open => setCarePlanDialog({ ...carePlanDialog, open })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="bg-red-50 p-4 rounded-lg">
              <p className="font-medium text-red-800">
                {carePlanDialog.patient?.citizen_name}
              </p>
              <p className="text-sm text-red-600">
                Risco: {carePlanDialog.patient?.risk_percentage}% em 10 anos
              </p>
            </div>
            <div className="space-y-2">
              <Label>Observações do Plano de Cuidado</Label>
              <Textarea
                placeholder="Descreva as orientações e acompanhamento necessário..."
                id="care-plan-notes"
              />
            </div>
            <div className="flex justify-end gap-3">
              ...
              <Button
                onClick={() => {
                  const notes =
                    document.getElementById("care-plan-notes")?.value || "";
                  createCarePlanMutation.mutate({
                    patientId: carePlanDialog.patient?.id,
                    notes,
                  });
                }}
              >
