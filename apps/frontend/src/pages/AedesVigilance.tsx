...
export default function AedesVigilance() {
  const [mapView, setMapView] = useState<string>('focos');
  ...
  const filteredFocuses = useMemo(() => {
    let data = focuses;
    if (filters.startDate) data = data.filter((f: any) => f.visit_date >= filters.startDate);
    if (filters.endDate) data = data.filter((f: any) => f.visit_date <= filters.endDate);
    if (filters.neighborhood !== 'all') data = data.filter((f: any) => f.neighborhood === filters.neighborhood);
    if (filters.focusType !== 'all') data = data.filter((f: any) => f.focus_type === filters.focusType);
    if (search) {
      const term = search.toLowerCase();
      data = data.filter((f: any) => f.address?.toLowerCase().includes(term) || f.neighborhood?.toLowerCase().includes(term));
    }
    return data;
  ...
  const filteredCases = useMemo(() => {
    let data = cases;
    if (filters.startDate) data = data.filter((c: any) => c.notification_date >= filters.startDate);
    if (filters.endDate) data = data.filter((c: any) => c.notification_date <= filters.endDate);
    if (filters.neighborhood !== 'all') data = data.filter((c: any) => c.neighborhood === filters.neighborhood);
    return data;
  }, [cases, filters]);

  const stats = useMemo(() => {
    const totalFocos = filteredFocuses.length;
    const focosEliminados = filteredFocuses.filter((f: any) => f.focus_eliminated).length;
    const imoveisVisitados = 1500; 
    const imoveisComFoco = new Set(filteredFocuses.map((f: any) => f.address)).size;
    const iip = imoveisVisitados > 0 ? ((imoveisComFoco / imoveisVisitados) * 100).toFixed(1) : '0.0';
    
    const dengueSuspeitos = filteredCases.filter((c: any) => c.disease_type === 'dengue' && c.status === 'suspeito').length;
    const dengueConfirmados = filteredCases.filter((c: any) => c.disease_type === 'dengue' && c.status === 'confirmado').length;
    const zikaCases = filteredCases.filter((c: any) => c.disease_type === 'zika').length;
    const chikungunyaCases = filteredCases.filter((c: any) => c.disease_type === 'chikungunya').length;
  ...
  const casesByDisease = [
    ...
    { name: 'Zika', suspeitos: filteredCases.filter((c: any) => c.disease_type === 'zika' && c.status === 'suspeito').length, confirmados: filteredCases.filter((c: any) => c.disease_type === 'zika' && c.status === 'confirmado').length },
    { name: 'Chikungunya', suspeitos: filteredCases.filter((c: any) => c.disease_type === 'chikungunya' && c.status === 'suspeito').length, confirmados: filteredCases.filter((c: any) => c.disease_type === 'chikungunya' && c.status === 'confirmado').length }
  ];

  const neighborhoods = [...new Set([...focuses.map((f: any) => f.neighborhood), ...cases.map((c: any) => c.neighborhood)].filter(Boolean))];

  const mapData = mapView === 'focos' 
    ? filteredFocuses.filter((f: any) => f.latitude && f.longitude)
    : filteredCases.filter((c: any) => c.latitude && c.longitude);

  const defaultCenter: [number, number] = [-14.8683986, -40.5862535];
  const mapCenter: [number, number] = mapData.length > 0 && mapData[0].latitude 
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredFocuses.map((f: any) => [
      f.visit_date,
      f.address,
      f.neighborhood,
              ...
              <div className="space-y-2">
                ...
                <Select value={filters.neighborhood} onValueChange={(v) => setFilters({ ...filters, neighborhood: v })}>
                  ...
                  <SelectContent>
                    ...
                    {neighborhoods.map((n: any) => <SelectItem key={n} value={n}>{n}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
        ...
        <Card className="shadow-lg border-0 bg-white/90">
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {paginatedFocuses.map((focus: any, idx: number) => (
