...
export default function ACSRanking() {
  ...
  const generateRankingMutation = useMutation({
    mutationFn: async () => {
      ...
      monthVisits.forEach(visit => {
        if (acsPoints[visit.acs_id]) {
          ...
          if (visit.conditions_found?.length > 0) {
            ...
            acsPoints[visit.acs_id].cases_monitored +=
              visit.conditions_found.length;
          }
        }
      });

      // Sort by points and assign positions
      const sorted = Object.values(acsPoints).sort(
      ...
      await Promise.all(
        existingRankings.map(r => trpc.ACSRanking.delete(r.id))
      );
      ...
      const newRankings = sorted.map((acs, idx) => ({
        ...
        variation: prevPositions[acs.acs_id]
          ? prevPositions[acs.acs_id] - (idx + 1)
          : 0,
        is_champion: idx === 0,
      }));
      ...
      if (sorted[0] && sorted[0].points > 0) {
        const existingChampion = champions.find(
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/70">
                  Gamificação e reconhecimento dos Agentes Comunitários
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 ${generateRankingMutation.isPending ? "animate-spin" : ""}`}
                />
