...
export default function Dashboard() {
  ...
  const {
    ...
    refetch,
  } = trpc.indicadores.filter.useQuery({
  ...
  // Fetch units
  const { data: units = [] } = trpc.healthUnits.filter.useQuery({
    active: true,
  });

  // Fetch teams
  const { data: teams = [] } = trpc.healthTeams.filter.useQuery({
    active: true,
  });
  ...
  const { data: qualityIssues = [] } = trpc.dataQuality.filter.useQuery({
    status: "aberto",
  });
  ...
  const indicatorsAboveTarget = Object.values(groupedIndicators).filter(
    (i: any) => {
  ...
  const avgQualityScore =
    Object.values(groupedIndicators).reduce(
      (sum: number, i: any) => sum + (i.quality_score || 90),
      0
    ) / Math.max(totalIndicators, 1);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Sistema de Monitoramento Previne Brasil
              </p>
            </div>
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
            ...
            <Card className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white border-0">
              ...
              <CardContent>
                <div className="text-3xl font-black">
                  {indicatorsAboveTarget}
                </div>
                <p className="text-xs opacity-75 mt-1">
                  {totalIndicators > 0
                    ? Math.round(
                        (indicatorsAboveTarget / totalIndicators) * 100
                      )
                    : 0}
                  % do total
                </p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Selecione uma categoria para filtrar os indicadores
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredIndicatorCodes.map(code => {
              ...
              return (
                ...
                >
                  <Card
                    className={`border-l-4 ${isAboveTarget ? "border-l-emerald-500" : "border-l-orange-500"}`}
                  >
                    <CardHeader className="pb-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min((indicator.result_percentage / target) * 100, 100)}%`,
                            }}
                          />
                        </div>
                      </div>
        ...
        {teamScores.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Top 10 equipes com melhor desempenho
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
